...
function preFilter(coin, aggressive=false) {
  ...
  if(longSetup){
    // ── Daily trend alignment (big bonus/reject) ──
    if(dailyTrend==='DOWN'&&dailyST==='DOWN') return {score:0, reason:'Daily BEAR — no LONGs'};
    ...
    if(dailyTrend==='UP'&&dailyST==='UP') return {score:0, reason:'Daily BULL — no SHORTs'};
