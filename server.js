...
// Trades closed by server guardian — frontend polls this every 30s for real-time sync
app.post('/api/trades/closed-since', async(req,res) => {
  if(!process.env.TURSO_DATABASE_URL) return res.json({ok:true, trades:[]});
  try {
    const {since} = req.body;
    const sinceISO = since ? new Date(since).toISOString() : new Date(Date.now()-24*60*60*1000).toISOString();
    const rows = await db.execute({
      sql:`SELECT * FROM active_trades
           WHERE status='CLOSED'
             AND close_reason IN ('TRAIL_SL','STOP_LOSS','TAKE_PROFIT','AI_EXIT','PARTIAL_TP1')
             AND closed_at > ?
...
async function smartTradeGuardian() {
  ...
  try {
    ...
    for(const trade of rows.rows){
      ...
      // ── 1. TRAILING STOP (same logic as frontend updateTrails) ──
      const gainPct = isLong ? (price-entry)/entry*100 : (entry-price)/entry*100;
      if(gainPct >= trailAct + 0.5){
        const lockPct  = Math.max(slPct*0.5, gainPct*0.30);
        const breakeven = isLong ? entry*1.002 : entry*0.998;
        let newSL;
        if(isLong){
          ...
          if(newSL > sl){
            ...
            console.log('[GUARDIAN] TRAIL '+trade.symbol+' SL raised → $'+sl.toFixed(6)+' (gain='+gainPct.toFixed(1)+'%)');
          ...
          if(newSL < sl){
            ...
            console.log('[GUARDIAN] TRAIL '+trade.symbol+' SL lowered → $'+sl.toFixed(6)+' (gain='+gainPct.toFixed(1)+'%)');
            // Live mode: update KuCoin stop order
            if(tradingMode==='live' && process.env.KUCOIN_API_KEY && trade.kucoin_sl_id) {
              try {
                await cancelKuOrder(trade.kucoin_sl_id).catch(()=>{});
