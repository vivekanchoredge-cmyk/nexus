    ...
    if(process.env.TURSO_DATABASE_URL&&Array.isArray(parsed.signals)&&parsed.signals.length>0){
      for(const sig of parsed.signals){
        ...
        try {
          ...
          // SYSTEM LEVEL SCORING: Claude × Historical Accuracy
          const systemConfidence = await calculateSystemScore(sig.symbol, sig.direction, sig.confidence || 7.0);
          
          // PER-SETUP KELLY: Individual setup sizing
          const setupKelly = await calculateSetupKelly(sig.symbol, sig.direction);
          
          await db.execute({
            sql:`INSERT INTO signals (symbol,signal,entry,tp,sl,confidence,reasoning,trade_type,exchange) VALUES (?,?,?,?,?,?,?,?,?)`,
            args:[sig.symbol,sig.direction,price,tpPrice,slPrice,systemConfidence||null,sig.reason||null,
                  finalTradeType, finalExchange]
          });
          console.log(`Signal saved: ${sig.symbol} ${sig.direction} | Claude=${sig.confidence} → System=${systemConfidence.toFixed(1)} | Kelly=${setupKelly.toFixed(2)}x`);
