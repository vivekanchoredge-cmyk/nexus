...
const USDT_PAIRS = COINS.map(c => c + 'USDT');
...
async function fetchBinancePrices() {
  const headers = BINANCE_KEY ? { 'X-MBX-APIKEY': BINANCE_KEY } : {};

  // 24hr ticker for all pairs
  ...
  const tickerMap = {};
  tickerResp.data.forEach(t => { tickerMap[t.symbol] = t; });

  const result = [];
  USDT_PAIRS.forEach((pair, idx) => {
    const sym = COINS[idx];
...
async function getMarketData() {
  if (Date.now() - marketCache.ts < 15000 && marketCache.data.length)
