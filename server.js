...
app.post('/file-ops', async (req, res) => {
  ...
  try {
    if(op === 'write') {
      ...
      // For Render: always use /tmp for writable paths
      const baseName = path.basename(filepath);
      const writePath = `/tmp/${baseName}`;
      
      // Ensure we have content (even if empty string)
      ...
      // Write the file
      await fsPromises.writeFile(writePath, fileContent, encoding);
      
      // Get file stats
      const stat = await fsPromises.stat(writePath);
      const lines = encoding === 'utf8' ? fileContent.split('\n').length : 0;
      
      console.log(`[NEXUS] File written: ${writePath} (${stat.size} bytes, ${lines} lines)`);
...
// ── File write endpoint (saves to /tmp) ────────────────────────
app.post('/file-create', async (req, res) => {
  ...
  try {
    ...
    console.log(`[NEXUS] File created: ${filePath} (${stat.size} bytes)`);
    ...
    res.json({
      ...
      size: stat.size,
      lines,
      message: `✅ File saved: ${safeName}`
    });
  } catch (e) {
    console.error('[NEXUS] File creation error:', e.message);
    res.status(500).json({ success: false, error: e.message });
  }
});
...
app.get('/files', async (req, res) => {
  try {
    ...
    const htmlFiles = files.filter(f => /\.(html|js|json|txt|css)$/i.test(f));
    
    const fileList = await Promise.all(
      htmlFiles.map(async (f) => {
        try {
          const stat = await fsPromises.stat(`/tmp/${f}`);
          return { name: f, size: stat.size, updated: stat.mtime };
        } catch { return null; }
      })
    );
    
    res.json({ files: fileList.filter(Boolean) });
  } catch (e) {
