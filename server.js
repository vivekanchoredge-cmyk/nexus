...
app.post('/api/order',async(req,res)=>{
  ...
  try {
    ...
    if(!kuOk && bxOk) {
      ...
      // Common coin → alternate based on symbol hash (fair distribution)
      if(global.bxOnlyCoins?.has(symbol)) {
        useBingX = true;
      } else if(global.kuOnlyCoins?.has(symbol)) {
        useBingX = false;
      } else {
        // Common coin — use symbol hash for fair alternation
        // This ensures consistent distribution across symbol names
        const hash = symbol.split('').reduce((a,b) => a + b.charCodeAt(0), 0);
        useBingX = hash % 2 === 0; // Even hash = BingX, Odd = KuCoin
      }
