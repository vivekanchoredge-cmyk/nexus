...
async function bxFuturesOrder(symbol, side, sizeUsdt, leverage=3) {
  // Set leverage first
...
async function kuFuturesOrder(symbol, side, sizeUsdt, leverage=3) {
...
function decideTradeType(coin) {
  ...
  // Futures: major coin + high volatility
  if(isMajor && (vr > 1.5 || atrPct > 2)) return { type:'futures', leverage:3 };
  // Futures: any coin with very high volatility spike
  if(vr > 2.0 && atrPct > 2.5) return { type:'futures', leverage:3 };
    ...
    if(process.env.TURSO_DATABASE_URL&&Array.isArray(parsed.signals)&&parsed.signals.length>0){
      for(const sig of parsed.signals){
        ...
        try {
          ...
          // Decide trade type for this signal
          ...
          await db.execute({
            ...
            args:[sig.symbol,sig.direction,price,tpPrice,slPrice,sig.confidence||null,sig.reason||null,
                  sigTradeDecision.type, (process.env.KUCOIN_API_KEY?'kucoin':'bingx')]
          });
  ...
  try {
    ...
    const result = await db.execute({
      ...
      args: [signal_id||null, symbol, direction, entry_price, tp_price||null, sl_price||null,
             ...
             trade_type||'spot', leverage||1, exchange||'kucoin']
    });
