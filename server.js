...
async function initDB() {
  ...
  `);
  // ── STRATEGY MARKETPLACE TABLE ────────────────────────────────────
  await db.execute(`
    CREATE TABLE IF NOT EXISTS strategies (
      id            INTEGER PRIMARY KEY AUTOINCREMENT,
      name          TEXT NOT NULL,
      description   TEXT,
      author        TEXT DEFAULT 'NEXUS',
  ...
  `);
  // Seed built-in strategies if not already there
  const existing = await db.execute({ sql:`SELECT COUNT(*) as cnt FROM strategies WHERE is_builtin=1`, args:[] });
  if(!existing.rows[0]?.cnt || existing.rows[0].cnt < 3) {
    const builtins = [
      { name:'Conservative Scalper', description:'Low risk, tight SL/TP. RSI+MACD confirmation. Best for sideways/low-vol markets.', params:JSON.stringify({sl_pct:1.5,tp_pct:3,score_threshold:70,aggressive:false,prefer_spot:true,indicators:['RSI','MACD','BB']}), win_rate:68 },
      { name:'Trend Rider', description:'Follows strong trends. Wider TP for bigger moves. Uses EMA stack + Supertrend.', params:JSON.stringify({sl_pct:2.5,tp_pct:6,score_threshold:60,aggressive:false,prefer_spot:false,indicators:['EMA','Supertrend','VWAP']}), win_rate:62 },
      { name:'Aggressive Swing', description:'High risk, high reward. Volatile coins, larger positions. For experienced traders.', params:JSON.stringify({sl_pct:3,tp_pct:9,score_threshold:50,aggressive:true,prefer_spot:false,indicators:['StochRSI','OBV','Pattern']}), win_rate:54 },
      { name:'Safe Accumulator', description:'DCA-style. Only buys on deep pullbacks with strong support. Very conservative.', params:JSON.stringify({sl_pct:1,tp_pct:2.5,score_threshold:80,aggressive:false,prefer_spot:true,indicators:['RSI','Support','VWAP']}), win_rate:74 }
    ];
    for(const s of builtins) {
      await db.execute({
        sql:`INSERT OR IGNORE INTO strategies (name,description,author,params,win_rate,is_builtin) VALUES (?,?,?,?,?,1)`,
        args:[s.name,s.description,'NEXUS TEAM',s.params,s.win_rate]
      });
    }
  }
  console.log('Turso DB ready (signals + active_trades + grid_bots + strategies)');
}
...
function requireAuth(req,res,next){
  ...
  if(req.path==='/api/webhook/tv') return next(); // TradingView uses secret in body
...
// ── GRID BOT ENGINE ───────────────────────────────────────────────
function calcGridLevels(lower, upper, numGrids) {
  const levels = [];
  const step = (upper - lower) / numGrids;
  for(let i = 0; i <= numGrids; i++) levels.push(parseFloat((lower + step * i).toFixed(8)));
  return levels;
}

async function gridBotRunner() {
  if(!process.env.TURSO_DATABASE_URL || !marketCache.length) return;
  try {
    const bots = await db.execute({ sql:`SELECT * FROM grid_bots WHERE status='RUNNING'`, args:[] });
    for(const bot of bots.rows) {
      const coin = marketCache.find(c => c.symbol === bot.symbol);
      if(!coin || !coin.price) continue;
      const price = coin.price;
      const levels = calcGridLevels(bot.lower_price, bot.upper_price, bot.num_grids);
      // Skip if price out of grid range
      if(price < bot.lower_price * 0.95 || price > bot.upper_price * 1.05) continue;

      // Get pending orders for this bot
      const orders = await db.execute({
        sql:`SELECT * FROM grid_orders WHERE bot_id=? AND status='PENDING' ORDER BY level_price ASC`,
        args:[bot.id]
      });

      // If no orders, initialize grid
      if(orders.rows.length === 0) {
        // Place buy orders below current price, sell orders above
        for(let i = 0; i < levels.length - 1; i++) {
          const lvl = levels[i];
          const orderType = lvl < price ? 'BUY' : 'SELL';
          await db.execute({
            sql:`INSERT INTO grid_orders (bot_id,type,level_price,level_idx,status) VALUES (?,?,?,?,?)`,
            args:[bot.id, orderType, lvl, i, 'PENDING']
          });
        }
        continue;
      }

      let pnlDelta = 0, cyclesCompleted = 0;
      for(const order of orders.rows) {
        // BUY order triggered (price dropped to/below level)
        if(order.type === 'BUY' && price <= order.level_price) {
          await db.execute({
            sql:`UPDATE grid_orders SET status='FILLED', filled_price=?, pnl=0 WHERE id=?`,
            args:[price, order.id]
          });
          // Create sell order at next level up
          const nextLevel = levels[order.level_idx + 1];
          if(nextLevel) {
            await db.execute({
              sql:`INSERT INTO grid_orders (bot_id,type,level_price,level_idx,status) VALUES (?,?,?,?,?)`,
              args:[bot.id, 'SELL', nextLevel, order.level_idx + 1, 'PENDING']
            });
          }
          // Live mode: place actual buy order
          if(bot.mode === 'live' && tradingMode === 'live') {
            const exchange = bot.exchange || 'kucoin';
            try {
              if(exchange === 'bingx' && BINGX_API_KEY) await bxPlaceOrder(bot.symbol, 'buy', bot.size_per_grid);
              else if(process.env.KUCOIN_API_KEY) await placeMarketOrder(bot.symbol, 'buy', bot.size_per_grid);
            } catch(e) { console.error('[GRID] Live buy failed:', e.message); }
          }
        }
        // SELL order triggered (price rose to/above level)
        else if(order.type === 'SELL' && price >= order.level_price) {
          const gridStep = (bot.upper_price - bot.lower_price) / bot.num_grids;
          const gridPnl = parseFloat(((gridStep / order.level_price) * bot.size_per_grid).toFixed(4));
          pnlDelta += gridPnl;
          cyclesCompleted++;
          await db.execute({
            sql:`UPDATE grid_orders SET status='FILLED', filled_price=?, pnl=? WHERE id=?`,
            args:[price, gridPnl, order.id]
          });
          // Create buy order at level below again
          const prevLevel = levels[order.level_idx - 1];
          if(prevLevel) {
            await db.execute({
              sql:`INSERT INTO grid_orders (bot_id,type,level_price,level_idx,status) VALUES (?,?,?,?,?)`,
              args:[bot.id, 'BUY', prevLevel, order.level_idx - 1, 'PENDING']
            });
          }
          ...
          if(bot.mode === 'live' && tradingMode === 'live') {
            const exchange = bot.exchange || 'kucoin';
            try {
              const qty = bot.size_per_grid / (coin.price || 1);
              if(exchange === 'bingx' && BINGX_API_KEY) await bxPlaceOrder(bot.symbol, 'sell', null, qty);
              else if(process.env.KUCOIN_API_KEY) await placeMarketOrder(bot.symbol, 'sell', null, qty);
            } catch(e) { console.error('[GRID] Live sell failed:', e.message); }
          }
        }
      }
      // Update bot stats
      if(pnlDelta !== 0 || cyclesCompleted > 0) {
        await db.execute({
          sql:`UPDATE grid_bots SET total_pnl=total_pnl+?, cycles=cycles+? WHERE id=?`,
          args:[pnlDelta, cyclesCompleted, bot.id]
        });
        if(cyclesCompleted > 0) console.log(`[GRID] ${bot.symbol} bot#${bot.id}: +${cyclesCompleted} cycles, PnL+${pnlDelta.toFixed(4)} USDT`);
      }
    }
  } catch(e) { console.error('[GRID] Runner error:', e.message); }
}
    ...
    if(process.env.TURSO_DATABASE_URL&&Array.isArray(parsed.signals)&&parsed.signals.length>0){
      for(const sig of parsed.signals){
        ...
        try {
          ...
          // Trade type: Claude ka suggestion prefer karo, fallback decideTradeType
          const sigCoin = marketCache.find(cc=>cc.symbol===sig.symbol);
          const sigTradeDecision = decideTradeType(sigCoin);
          // BUG WAS: sigTradeDecision.type always overwrote Claude's trade_type
          // FIX: Claude ka trade_type use karo agar available ho
          const finalTradeType = sig.trade_type || sigTradeDecision.type || 'spot';
...
app.post('/api/trades/open', async(req,res) => {
  ...
  try {
    ...
    // trade_type default: 'spot' — but never let it be undefined/null
...
async function runBacktestForCoin(symbol, days=60, slPct=2, tpPct=5, aggressive=false) {
  const mins15 = Math.min(Math.ceil(days*24*4), 480);
  const mins60  = Math.min(Math.ceil(days*24),   480);
  const mins240 = Math.min(Math.ceil(days*6),    480);
  const mins1d  = Math.min(days, 365);

  let m15=[], h1=[], h4=[], d1=[];
  try {
    m15 = await loadCandles(symbol,'15min',mins15); await sleep(300);
    h1  = await loadCandles(symbol,'1hour', mins60); await sleep(300);
    h4  = await loadCandles(symbol,'4hour',mins240); await sleep(300);
    d1  = await loadCandles(symbol,'1day',  mins1d);
  } catch(e) { return null; }
  ...
  while(i < m15.length - 1) {
    ...
    const cur_h4 = h4.filter(c=>c.time <= curTime);
    const cur_d1 = d1.filter(c=>c.time <= curTime);
    if(cur_h1.length < 26 || cur_h4.length < 10) continue;
    const snap15 = tfSnapshot(cur15);
    const snapH1 = tfSnapshot(cur_h1);
    const snapH4 = tfSnapshot(cur_h4);
    const snapD1 = cur_d1.length >= 26 ? tfSnapshot(cur_d1) : null;
    if(!snap15||!snapH1||!snapH4) continue;
    const price = m15[i-1].close;
    // Track average ATR for futures decision
    if(snap15.atr) { totalAtr += snap15.atr; atrCount++; }

    if(!inTrade) {
      const coin = { symbol, price, m15:snap15, h1:snapH1, h4:snapH4, d1:snapD1 };
...
// ── TRADINGVIEW WEBHOOK ───────────────────────────────────────────
// No auth middleware — TradingView uses secret in body
// JSON format: { secret, symbol, action, price, sl_pct, tp_pct, size_usdt }
app.post('/api/webhook/tv', async(req,res) => {
  const { secret, symbol, action, price, sl_pct=2, tp_pct=5, size_usdt=100, comment } = req.body || {};
  const expectedSecret = process.env.NEXUS_WEBHOOK_SECRET || process.env.NEXUS_PASSWORD || 'nexus123';
  if(secret !== expectedSecret) return res.status(401).json({ok:false,error:'Invalid webhook secret'});
  if(!symbol || !action) return res.status(400).json({ok:false,error:'symbol and action required'});

  const sym = symbol.replace(/USDT?$/, '').replace(/-/g,'').toUpperCase();
  const direction = action.toUpperCase() === 'BUY' ? 'LONG' : action.toUpperCase() === 'SELL' ? 'SHORT' : null;
  if(!direction) return res.status(400).json({ok:false,error:'action must be buy or sell'});

  const coin = marketCache.find(c => c.symbol === sym);
  const entryPrice = price || coin?.price || null;
  const slPrice = entryPrice ? (direction==='LONG' ? entryPrice*(1-sl_pct/100) : entryPrice*(1+sl_pct/100)) : null;
  const tpPrice = entryPrice ? (direction==='LONG' ? entryPrice*(1+tp_pct/100) : entryPrice*(1-tp_pct/100)) : null;

  console.log(`[WEBHOOK] TradingView: ${sym} ${direction} @ $${entryPrice} (${comment||'no comment'})`);
  ...
  if(process.env.TURSO_DATABASE_URL) {
    try {
      await db.execute({
        sql:`INSERT INTO signals (symbol,signal,entry,tp,sl,confidence,reasoning,trade_type,exchange) VALUES (?,?,?,?,?,?,?,?,?)`,
        args:[sym, direction, entryPrice, tpPrice, slPrice, 75, `TradingView webhook: ${comment||action}`, 'spot', 'webhook']
      });
    } catch(e) { console.error('[WEBHOOK] DB error:', e.message); }
  }

  // Auto-execute if live mode and exchange configured
  let executed = false;
  if(tradingMode === 'live') {
    try {
      const kuOk = !!(process.env.KUCOIN_API_KEY && process.env.KUCOIN_API_SECRET && process.env.KUCOIN_PASSPHRASE);
      ...
      if(kuOk || bxOk) {
        const side = direction === 'LONG' ? 'buy' : 'sell';
        if(bxOk && !kuOk) await bxPlaceOrder(sym, side, size_usdt);
        else if(kuOk) await placeMarketOrder(sym, side, size_usdt);
        executed = true;
        console.log(`[WEBHOOK] Trade executed: ${sym} ${direction} $${size_usdt}`);
      }
    } catch(e) { console.error('[WEBHOOK] Execution failed:', e.message); }
  }

  res.json({ ok:true, symbol:sym, direction, entry:entryPrice, sl:slPrice, tp:tpPrice, executed, mode:tradingMode });
});
...
app.get('/api/webhook/history', async(req,res) => {
  ...
  try {
    const rows = await db.execute({
      sql:`SELECT * FROM signals WHERE exchange='webhook' ORDER BY id DESC LIMIT 50`,
      args:[]
    });
    res.json({ok:true, signals:rows.rows});
  } catch(e) { res.status(500).json({ok:false,error:e.message}); }
...
// ── GRID BOT ROUTES ───────────────────────────────────────────────
app.post('/api/grid/create', async(req,res) => {
  if(!process.env.TURSO_DATABASE_URL) return res.status(500).json({ok:false,error:'DB not configured'});
  const { symbol, lower_price, upper_price, num_grids=10, size_per_grid=50, exchange='kucoin' } = req.body;
  if(!symbol || !lower_price || !upper_price) return res.status(400).json({ok:false,error:'symbol, lower_price, upper_price required'});
  if(lower_price >= upper_price) return res.status(400).json({ok:false,error:'lower_price must be < upper_price'});
  if(num_grids < 2 || num_grids > 50) return res.status(400).json({ok:false,error:'num_grids must be 2-50'});
  try {
    const result = await db.execute({
      sql:`INSERT INTO grid_bots (symbol,lower_price,upper_price,num_grids,size_per_grid,exchange,mode,status) VALUES (?,?,?,?,?,?,?,?)`,
      args:[symbol.toUpperCase(), parseFloat(lower_price), parseFloat(upper_price), parseInt(num_grids), parseFloat(size_per_grid), exchange, tradingMode, 'RUNNING']
    });
    console.log(`[GRID] New bot created: ${symbol} $${lower_price}-$${upper_price} ${num_grids} grids`);
    res.json({ok:true, bot_id:Number(result.lastInsertRowid)});
  } catch(e) { res.status(500).json({ok:false,error:e.message}); }
});
...
app.post('/api/grid/stop', async(req,res) => {
  ...
  try {
    await db.execute({sql:`UPDATE grid_bots SET status='STOPPED' WHERE id=?`, args:[bot_id]});
    await db.execute({sql:`UPDATE grid_orders SET status='CANCELLED' WHERE bot_id=? AND status='PENDING'`, args:[bot_id]});
    res.json({ok:true});
  } catch(e) { res.status(500).json({ok:false,error:e.message}); }
});

app.delete('/api/grid/:id', async(req,res) => {
  if(!process.env.TURSO_DATABASE_URL) return res.json({ok:false,error:'DB not configured'});
  try {
    await db.execute({sql:`UPDATE grid_bots SET status='DELETED' WHERE id=?`, args:[req.params.id]});
    res.json({ok:true});
  } catch(e) { res.status(500).json({ok:false,error:e.message}); }
});

app.get('/api/grid/list', async(req,res) => {
  if(!process.env.TURSO_DATABASE_URL) return res.json({ok:true, bots:[]});
  try {
    const bots = await db.execute({
      sql:`SELECT g.*, (SELECT COUNT(*) FROM grid_orders WHERE bot_id=g.id AND status='FILLED') as filled_orders
           FROM grid_bots g WHERE g.status != 'DELETED' ORDER BY g.created_at DESC`,
      args:[]
    });
    // Add current price and profit % to each bot
    const result = bots.rows.map(bot => {
      const coin = marketCache.find(c => c.symbol === bot.symbol);
      const currentPrice = coin?.price || null;
      const totalInvested = bot.num_grids * bot.size_per_grid;
      const pnlPct = totalInvested > 0 ? parseFloat(((bot.total_pnl / totalInvested) * 100).toFixed(2)) : 0;
      const inRange = currentPrice ? (currentPrice >= bot.lower_price && currentPrice <= bot.upper_price) : false;
      return { ...bot, currentPrice, pnlPct, totalInvested, inRange };
    });
    res.json({ok:true, bots:result});
  } catch(e) { res.status(500).json({ok:false,error:e.message}); }
});

app.get('/api/grid/orders/:bot_id', async(req,res) => {
  if(!process.env.TURSO_DATABASE_URL) return res.json({ok:true, orders:[]});
  try {
    const orders = await db.execute({
      sql:`SELECT * FROM grid_orders WHERE bot_id=? ORDER BY level_price DESC LIMIT 100`,
      args:[req.params.bot_id]
    });
...
app.get('/api/strategies', async(req,res) => {
  if(!process.env.TURSO_DATABASE_URL) return res.json({ok:true, strategies:[]});
  try {
    const rows = await db.execute({
      sql:`SELECT * FROM strategies WHERE is_builtin=1 OR author='USER' ORDER BY win_rate DESC`,
      args:[]
    });
    res.json({ok:true, strategies:rows.rows});
  } catch(e) { res.status(500).json({ok:false,error:e.message}); }
});
...
app.post('/api/strategies/save', async(req,res) => {
  ...
  if(!name || !params) return res.status(400).json({ok:false,error:'name and params required'});
  try {
    // Run quick backtest to get real win rate for this strategy
    const paramObj = typeof params === 'string' ? JSON.parse(params) : params;
    const testCoins = marketCache.slice(0,5).map(c=>c.symbol);
    let totalWins=0, totalTrades=0;
    for(const sym of testCoins) {
      const r = await runBacktestForCoin(sym, 30, paramObj.sl_pct||2, paramObj.tp_pct||5, paramObj.aggressive||false);
      if(r) { totalWins += r.wins; totalTrades += r.total; }
    }
    const realWR = totalTrades > 5 ? parseFloat((totalWins/totalTrades*100).toFixed(1)) : 0;
    const result = await db.execute({
      sql:`INSERT INTO strategies (name,description,author,params,win_rate,total_trades) VALUES (?,?,?,?,?,?)`,
      args:[name, description||'', 'USER', typeof params==='string'?params:JSON.stringify(params), realWR, totalTrades]
    });
    res.json({ok:true, strategy_id:Number(result.lastInsertRowid), win_rate:realWR});
  } catch(e) { res.status(500).json({ok:false,error:e.message}); }
});
...
app.post('/api/strategies/apply', async(req,res) => {
  ...
  if(!process.env.TURSO_DATABASE_URL) return res.json({ok:false,error:'DB not configured'});
  try {
    const rows = await db.execute({sql:`SELECT * FROM strategies WHERE id=?`, args:[strategy_id]});
    if(!rows.rows.length) return res.status(404).json({ok:false,error:'Strategy not found'});
    const strategy = rows.rows[0];
    const params = JSON.parse(strategy.params);
    res.json({ok:true, strategy:strategy.name, params});
  } catch(e) { res.status(500).json({ok:false,error:e.message}); }
});
...
app.delete('/api/strategies/:id', async(req,res) => {
  ...
  try {
    ...
    await db.execute({sql:`DELETE FROM strategies WHERE id=? AND is_builtin=0`, args:[req.params.id]});
    res.json({ok:true});
  } catch(e) { res.status(500).json({ok:false,error:e.message}); }
});
...
app.post('/api/backtest/report', async(req,res) => {
  try {
    const { symbol, days=90, sl_pct=2, tp_pct=5, aggressive=false } = req.body;
    if(!symbol) return res.status(400).json({ok:false,error:'symbol required'});

    const result = await runBacktestForCoin(symbol, days, sl_pct, tp_pct, aggressive);
    if(!result) return res.status(400).json({ok:false,error:'Not enough data'});
    ...
    const sharpe = stdDev > 0 ? parseFloat((mean/stdDev*Math.sqrt(365/days)).toFixed(2)) : 0;
    ...
    for(const t of trades) {
      ...
      byReason[t.reason].pnl = parseFloat((byReason[t.reason].pnl + t.pnl).toFixed(2));
    }
...
// Daily PnL — last 30 days grouped
app.get('/api/performance/daily', async(req,res) => {
  if(!process.env.TURSO_DATABASE_URL) return res.json({ok:true, days:[]});
  try {
    const rows = await db.execute({
      sql:`SELECT
...
// Best/Worst coins
app.get('/api/performance/coins', async(req,res) => {
  if(!process.env.TURSO_DATABASE_URL) return res.json({ok:true, coins:[]});
  try {
    const rows = await db.execute({
      sql:`SELECT
             symbol,
             ...
             ROUND(SUM(pnl_pct),2) as total_pnl,
             ROUND(AVG(pnl_pct),2) as avg_pnl,
             ROUND(CAST(SUM(CASE WHEN pnl_pct>0 THEN 1 ELSE 0 END) AS FLOAT)/COUNT(*)*100,1) as wr
           FROM active_trades
    ...
    res.json({ok:true, coins: rows.rows});
  } catch(e) { res.status(500).json({ok:false,error:e.message}); }
});

// Hourly performance — which hours are most profitable
app.get('/api/performance/hours', async(req,res) => {
  if(!process.env.TURSO_DATABASE_URL) return res.json({ok:true, hours:[]});
  try {
    const rows = await db.execute({
      sql:`SELECT
    ...
    res.json({ok:true, hours: rows.rows});
  } catch(e) { res.status(500).json({ok:false,error:e.message}); }
});

// Overall summary stats — single call for dashboard header
app.get('/api/performance/summary', async(req,res) => {
  if(!process.env.TURSO_DATABASE_URL) return res.json({ok:true, summary:{}});
  try {
    const [overall, today, week, streak] = await Promise.all([
      db.execute({sql:`SELECT COUNT(*) as t, SUM(CASE WHEN pnl_pct>0 THEN 1 ELSE 0 END) as w, ROUND(SUM(pnl_pct),2) as pnl FROM active_trades WHERE status='CLOSED' AND pnl_pct IS NOT NULL`,args:[]}),
      db.execute({sql:`SELECT ROUND(SUM(pnl_pct),2) as pnl, COUNT(*) as t FROM active_trades WHERE status='CLOSED' AND pnl_pct IS NOT NULL AND DATE(closed_at)=DATE('now')`,args:[]}),
      db.execute({sql:`SELECT ROUND(SUM(pnl_pct),2) as pnl, COUNT(*) as t FROM active_trades WHERE status='CLOSED' AND pnl_pct IS NOT NULL AND closed_at>=DATE('now','-7 days')`,args:[]}),
      db.execute({sql:`SELECT pnl_pct FROM active_trades WHERE status='CLOSED' AND pnl_pct IS NOT NULL ORDER BY closed_at DESC LIMIT 10`,args:[]})
    ]);
    // Current streak
    let curStreak=0, streakType='';
    for(const r of streak.rows){
      if(curStreak===0){ streakType=r.pnl_pct>0?'WIN':'LOSS'; curStreak=1; }
      else if((r.pnl_pct>0&&streakType==='WIN')||(r.pnl_pct<=0&&streakType==='LOSS')) curStreak++;
      else break;
    }
    const o=overall.rows[0], td=today.rows[0], wk=week.rows[0];
    res.json({ok:true, summary:{
      total_trades:o.t||0,
      total_wins:o.w||0,
      total_pnl:o.pnl||0,
      win_rate: o.t>0 ? parseFloat(((o.w/o.t)*100).toFixed(1)) : 0,
      today_pnl:td.pnl||0, today_trades:td.t||0,
      week_pnl:wk.pnl||0,  week_trades:wk.t||0,
      streak:curStreak, streak_type:streakType
    }});
  } catch(e) { res.status(500).json({ok:false,error:e.message}); }
});
...
app.post('/api/backtest/compare', async(req,res) => {
  try {
    const { symbols=[], days=60, sl_pct=2, tp_pct=5 } = req.body;
    const tops = symbols.length ? symbols : marketCache.slice(0,10).map(c=>c.symbol);
    const results = [];
    for(const sym of tops.slice(0,15)) {
      const r = await runBacktestForCoin(sym, days, sl_pct, tp_pct, false);
      if(r) results.push({ symbol:sym, wr:r.wr, totalPnl:r.totalPnl, trades:r.total, tradeType:r.tradeType });
      await sleep(200);
    }
...
app.listen(PORT,async()=>{
  ...
  // ── GRID BOT RUNNER (every 30s) ─────────────────────────────────
  setInterval(gridBotRunner, 30*1000);
  console.log('[GRID] Grid Bot Runner started — checking every 30s');
