...
async function calcKellySize(baseCapital) {
  ...
  try {
    ...
    const pct = Math.max(kellyConfig.minPct, Math.min(kellyConfig.maxPct, halfKelly * 100));
    const sizeUsdt = parseFloat((baseCapital * pct / 100).toFixed(2));
    console.log(`[KELLY] WR=${(wr*100).toFixed(1)}% avgWin=${avgWin.toFixed(2)}% avgLoss=${avgLoss.toFixed(2)}% kelly=${(kelly*100).toFixed(2)}% → ${pct.toFixed(2)}% = $${sizeUsdt}`);
...
app.post('/api/trades/open', async(req,res) => {
  ...
  try {
    ...
    if(!riskResult.ok) {
      ...
      sendTelegram(`🚫 <b>TRADE BLOCKED</b>\n${symbol} ${direction}\nReason: ${riskResult.reason}`);
...
let approvedCoins = {};
let autoBacktestEnabled = true;   // User toggle for auto-backtest feature
let manualBacktestEnabled = true; // User toggle for manual backtest feature
...
async function autoBacktest() {
  if(!autoBacktestEnabled) {
    console.log('[BACKTEST] Auto-backtest disabled by user, skipping');
    return;
  }
...
app.post('/api/backtest', async(req,res) => {
  // Check if manual backtest is enabled
  if(!manualBacktestEnabled) {
    console.log('[BACKTEST] Manual backtest disabled, rejecting request');
    return res.status(403).json({ok:false, error:'Manual backtest is disabled'});
  }
...
// Toggle auto-backtest on/off
app.post('/api/backtest/auto-toggle', (req,res) => {
  const {enabled} = req.body;
  if(typeof enabled === 'boolean'){
    autoBacktestEnabled = enabled;
    console.log(`[BACKTEST] Auto-backtest ${enabled ? 'ENABLED (6h interval)' : 'DISABLED'}`);
    res.json({ok:true, enabled: autoBacktestEnabled});
  } else {
    res.status(400).json({ok:false, error:'enabled must be boolean'});
  }
});

// Toggle manual backtest on/off
app.post('/api/backtest/manual-toggle', (req,res) => {
  const {enabled} = req.body;
  if(typeof enabled === 'boolean'){
    manualBacktestEnabled = enabled;
    console.log(`[BACKTEST] Manual backtest ${enabled ? 'ENABLED' : 'DISABLED'}`);
    res.json({ok:true, enabled: manualBacktestEnabled});
  } else {
    res.status(400).json({ok:false, error:'enabled must be boolean'});
  }
});
...
app.post('/api/risk/config', (req,res)=>{
  ...
  sendTelegram(`⚙️ <b>Risk Config Updated</b>\nDaily Limit: $${riskConfig.dailyLossLimit} | Max Pos: ${riskConfig.maxOpenPositions} | Max DD: ${riskConfig.maxDrawdown}% | ATR Mult: ${riskConfig.atrSLMultiplier}`);
...
app.get('/api/telegram/test', async(req,res)=>{
  ...
  await sendTelegram('🤖 <b>NEXUS PRO</b> connected! Notifications active ✅');
...
async function startGridBot(symbol, low, high, grids, totalUsdt) {
  ...
  sendTelegram(`🔲 <b>GRID BOT STARTED</b>
...
function stopGridBot(symbol) {
  ...
  sendTelegram(`🔲 <b>GRID BOT STOPPED</b>
...
async function runGridChecker() {
  for(const [sym, g] of Object.entries(gridBots)) {
    ...
    if(price < g.low * 0.97 || price > g.high * 1.03) {
      ...
      sendTelegram(`⚠️ <b>GRID AUTO-STOPPED</b>
...
app.post('/api/webhook/tv', async(req,res)=>{
  try {
    ...
    try {
      await db.execute({
        sql:`INSERT INTO signals (symbol,signal,entry,tp,sl,confidence,reasoning,outcome,trade_type) VALUES (?,?,?,?,?,?,?,'PENDING','spot')`,
        args:[symbol, dir, coinPrice, tpPrice, slPrice, confidence, `TradingView webhook: ${action}`]
      });
    } catch(e){}

    sendTelegram(`📡 <b>TV WEBHOOK</b> ${dir} <b>${symbol}</b>
...
async function closeTrade(trade, exitPrice, reason) {
  ...
  // Update signals table — also update trade_type so futures WR tracked correctly
  if(pnlPct !== null) {
    ...
    await db.execute({
      ...
      args: [outcome, parseFloat(exitPrice.toFixed(8)), pnlPct, trade.trade_type||'spot', trade.symbol, trade.symbol]
    });
  ...
  function scheduleDailyReport(){
    ...
    setTimeout(async()=>{
      if(notifConfig.onDailyReport && TG_TOKEN && TG_CHAT){
        try {
          const r = await fetch ? null : null; // use internal logic
