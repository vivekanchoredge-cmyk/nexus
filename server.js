  ...
  try {
    ...
    if(!kuOk && bxOk) {
      ...
      if(global.bxOnlyCoins?.has(symbol)) {
        ...
        // Common coin — cycle count se alternate karo
        useBingX = (Date.now() / 1000 / 60) % 2 < 1; // Har minute alternate
      }
...
app.post('/api/trades/journal/entry', async(req,res) => {
  ...
  try {
    ...
    res.json({ok:true, journal_id: result.lastInsertRowid});
  } catch(e) {
...
// 🔥 FIX: Notify missed closed trades when bot wakes from sleep
async function notifyMissedTrades() {
  try {
    const missed = await db.execute({
      sql: `SELECT id, symbol, direction, entry_price, close_price, pnl_pct 
            FROM active_trades 
            WHERE status='closed' 
            AND telegram_notified IS NULL 
            LIMIT 10`
    });
    
    if(missed.rows && missed.rows.length > 0) {
      console.log(`[NOTIFY] Found ${missed.rows.length} missed closed trades`);
      for(const trade of missed.rows) {
        const pnlColor = trade.pnl_pct >= 0 ? '✅' : '❌';
        const pnlStr = trade.pnl_pct >= 0 ? `+${trade.pnl_pct.toFixed(2)}` : `${trade.pnl_pct.toFixed(2)}`;
        
        await sendTelegramFiltered(
          `🔔 <b>MISSED NOTIFICATION (Sleep Period)</b>\n${pnlColor} <b>${trade.symbol}</b> ${trade.direction}\nEntry: $${parseFloat(trade.entry_price).toFixed(4)} → Exit: $${parseFloat(trade.close_price).toFixed(4)}\nPnL: <b>${pnlStr}%</b>`,
          'trade_close'
        );
        
        // Mark as notified
        await db.execute({
          sql: `UPDATE active_trades SET telegram_notified=1 WHERE id=?`,
          args: [trade.id]
        });
      }
    }
  } catch(e) {
    console.error('[NOTIFY MISSED] Error:', e.message);
  }
}
...
app.listen(PORT,async()=>{
  ...
  await initDB();
  await initialLoad();
  // 🔥 Check for missed notifications from sleep period
  setTimeout(() => notifyMissedTrades(), 3000);
