...
async function initDB() {
  ...
  try { await db.execute(`ALTER TABLE active_trades ADD COLUMN tp2_hit INTEGER DEFAULT 0`); } catch(e){}
  // Portfolio state table — persist cash balance across restarts
  await db.execute(`
  ...
  `);
  // trade_journal table — detailed per-trade log with reasoning
  await db.execute(`
...
async function loadMemory(symbol) {
  ...
  try {
    ...
    // Split into avoid/caution/good for Claude clarity
    const avoidCoins = statsRows.rows.filter(r => r.wins === 0);
    const goodCoins  = statsRows.rows.filter(r => r.total > 0 && ((r.wins/r.total)*100) >= 65);
    const statsLines = statsRows.rows.map(r=>{
      const wr = r.total>0 ? ((r.wins/r.total)*100).toFixed(0) : '?';
      let flag = '';
      if (r.wins === 0)                          flag = '🚫AVOID — 0 wins ever';
      else if (parseFloat(wr) < 35)              flag = '⚠️SKIP — poor WR';
      else if (parseFloat(wr) >= 80)             flag = '🏆STRONG — high WR';
      else if (parseFloat(wr) >= 65)             flag = '✅GOOD';
      return `  ${r.symbol}: ${r.wins}W/${r.losses}L/${r.total}T wr=${wr}% avg_pnl=${r.avg_pnl}% ${flag}`;
    }).join('\n');
    // Build explicit AVOID and STRONG lists for Claude prompt
    const avoidList = avoidCoins.length > 0
      ? `\n🚫 COINS TO AVOID (0% WR — never won): ${avoidCoins.map(r=>r.symbol).join(', ')}`
      : '';
    const strongList = goodCoins.length > 0
      ? `\n🏆 STRONG COINS (65%+ WR): ${goodCoins.map(r=>`${r.symbol}(${((r.wins/r.total)*100).toFixed(0)}%)`).join(', ')}`
      : '';
    ...
    return `\n\nYOUR MEMORY (only verified WIN/LOSS data — no NULL/PENDING noise):\n1. ${symbol} HISTORY (last 200):${streakStr}\n${symLines||'  No history yet'}\n2. ALL COIN WIN RATES (from ${statsRows.rows.length} tracked coins):${avoidList}${strongList}\n${statsLines||'  No completed trades yet'}\n3. OPEN PENDING TRADES: ${openTrades}${typeStats}\nRULES:\n- 🚫 AVOID coins: give confidence ≤4.0, do NOT trade unless exceptional 3+ indicator setup\n- ⚠️ SKIP coins (<35% WR): give confidence ≤5.5\n- ✅ GOOD coins (≥65% WR): add +0.5 confidence bonus\n- Same losing pattern on same coin = SKIP\n- 2+ consecutive losses on coin = REDUCE SIZE\n- futures wr<50% = prefer spot`;
  } catch(e) {
...
function trend(closes){
  if(!closes||closes.length<26) return 'SIDE';
  ...
  if(e50){
    ...
    // Relaxed: EMA9>EMA21 aur EMA21 EMA50 ke aas paas
    if(e9>e21&&e21>=e50*0.99) return 'UP';
    if(e9<e21&&e21<=e50*1.01) return 'DOWN';
  } else {
    // Sirf 2 EMA
    if(e9>e21) return 'UP';
...
function preFilter(coin, aggressive=false, futuresMode=false) {
  ...
  if(!aggressive){
    // SIDE+SIDE block sirf tab jab m15 bhi SIDE ho — thoda relax karo
...
async function loadBxCandles(sym, interval, limit=100){
  // BingX spot candles API
  const intervalMap = {'15min':'15m','1hour':'1h','4hour':'4h','1day':'1d'};
  const bxInterval = intervalMap[interval] || '15m';
  try {
    const r = await axios.get('https://open-api.bingx.com/openApi/spot/v2/market/kline', {
      params: { symbol: sym+'-USDT', interval: bxInterval, limit, timestamp: Date.now() },
      timeout: 12000
    });
    if(String(r.data.code) !== '0') throw new Error(r.data.msg);
  ...
  } catch(e) { throw new Error(`BingX candles ${sym} ${interval}: ${e.message}`); }
}
...
async function initialLoad(){
  ...
  try {
    ...
    kuCoins=r.data.data.ticker
      .filter(t=>t.symbol.endsWith('-USDT')&&!exclude.some(e=>t.symbol.replace('-USDT','').includes(e))&&parseFloat(t.volValue)>500000)
  ...
  // Track KuCoin-only coins
  global.kuOnlyCoins = new Set(kuCoins.filter(s=>!bxCoins.includes(s)));
  ...
  for(const sym of activeSymbols){
    ...
    const isBxOnly = global.bxOnlyCoins && global.bxOnlyCoins.has(sym);
    const loader = isBxOnly ? loadBxCandles : loadCandles;
    try {
      store[sym].m15=await loader(sym,'15min',100); await sleep(300);
      store[sym].h1=await loader(sym,'1hour',100);  await sleep(300);
      store[sym].h4=await loader(sym,'4hour',60);   await sleep(300);
      store[sym].d1=await loader(sym,'1day',60);    await sleep(300);
      loaded++;
    } catch(e){ console.warn(`Candle load failed ${sym}(${isBxOnly?'BX':'KU'}): ${e.message}`); }
  }
...
function buildCache(){
  ...
  const _ku=marketCache.filter(c=>!c.isBingX);
  const _bx=marketCache.filter(c=>c.isBingX);
  const _mix=[];const _ml=Math.max(_ku.length,_bx.length);
  for(let i=0;i<_ml;i++){if(_ku[i])_mix.push(_ku[i]);if(_bx[i])_mix.push(_bx[i]);}
  marketCache=_mix;
}
...
// 1. Get price from both exchanges (for best price routing)
async function getBestPrice(symbol, side) {
  try {
    const results = await Promise.allSettled([
      // KuCoin price
      axios.get(`https://api.kucoin.com/api/v1/market/orderbook/level1?symbol=${symbol}-USDT`, {timeout:3000})
        .then(r => ({ exchange: 'kucoin', price: parseFloat(r.data.data?.price || 0) })),
      // BingX price
      axios.get(`https://open-api.bingx.com/openApi/spot/v1/ticker/price?symbol=${symbol}-USDT&timestamp=${Date.now()}`, {timeout:3000})
        .then(r => ({ exchange: 'bingx', price: parseFloat(r.data.data?.trades?.[0]?.price || r.data.data?.price || 0) }))
    ]);
    ...
    // For BUY: want lowest price (cheapest)
    // For SELL: want highest price (most profit)
    ...
    console.error('[BEST PRICE ERROR]', e.message);
    return null;
  }
}

// 2. Slippage logger
function logSlippage(symbol, exchange, expectedPrice, filledPrice) {
  try {
    if (!expectedPrice || !filledPrice) return;
    const slippage = Math.abs((filledPrice - expectedPrice) / expectedPrice) * 100;
    if (slippage > 0.3) {
      console.warn(`[SLIPPAGE WARNING] ${symbol} ${exchange}: expected=${expectedPrice} filled=${filledPrice} slip=${slippage.toFixed(3)}%`);
    } else {
      console.log(`[SLIPPAGE OK] ${symbol} ${exchange}: ${slippage.toFixed(3)}%`);
    }
  } catch(e) {}
}

// 3. Smart retry wrapper for KuCoin orders
async function placeMarketOrderWithRetry(symbol, side, sizeUsdt, coinQty=null, retries=2) {
  for (let i = 0; i <= retries; i++) {
    try {
      const start = Date.now();
      const orderId = await placeMarketOrder(symbol, side, sizeUsdt, coinQty);
      const latency = Date.now() - start;
      console.log(`[KUCOIN EXEC] ${symbol} ${side} $${sizeUsdt} | latency=${latency}ms | attempt=${i+1}`);
      return orderId;
    } catch(e) {
      console.error(`[KUCOIN RETRY ${i+1}/${retries+1}] ${symbol}: ${e.message}`);
      if (i < retries) await new Promise(r => setTimeout(r, 1000 * (i+1))); // Wait 1s, 2s
    }
  }
  console.error(`[KUCOIN FAIL] ${symbol} - all ${retries+1} attempts failed`);
  return null;
}

// 4. Smart retry wrapper for BingX futures orders
async function bxFuturesOrderWithRetry(symbol, side, sizeUsdt, leverage=5, retries=2) {
  for (let i = 0; i <= retries; i++) {
    try {
      const start = Date.now();
      const result = await bxFuturesOrder(symbol, side, sizeUsdt, leverage);
      const latency = Date.now() - start;
      console.log(`[BINGX EXEC] ${symbol} ${side} $${sizeUsdt} lev=${leverage}x | latency=${latency}ms | attempt=${i+1}`);
      return result;
    } catch(e) {
      console.error(`[BINGX RETRY ${i+1}/${retries+1}] ${symbol}: ${e.message}`);
      if (i < retries) await new Promise(r => setTimeout(r, 1000 * (i+1)));
    }
  }
  console.error(`[BINGX FAIL] ${symbol} - all ${retries+1} attempts failed`);
  return null;
}

// 5. Smart execution: picks best price exchange, retries on fail
async function smartExecute(symbol, direction, sizeUsdt, tradeType='futures', leverage=5) {
  ...
  try {
    ...
    // Decide exchange from signal (passed in) or best price
    let useExchange = null;
    if (kuOk && bxOk && best) {
      useExchange = best.exchange;
      console.log(`[SMART ROUTE] ${symbol} ${direction}: Best price on ${useExchange.toUpperCase()} @ ${best.price}`);
    } else if (bxOk) {
      useExchange = 'bingx';
    } else {
      useExchange = 'kucoin';
    ...
    if (useExchange === 'bingx') {
      if (tradeType === 'futures') {
        orderId = await bxFuturesOrderWithRetry(symbol, side, sizeUsdt, leverage);
      } else {
        orderId = await bxFuturesOrderWithRetry(symbol, side, sizeUsdt, 1); // spot = 1x
      }
      // Fallback to KuCoin if BingX fails
      if (!orderId && kuOk) {
        console.log(`[FALLBACK] BingX failed → trying KuCoin`);
        orderId = await placeMarketOrderWithRetry(symbol, side.toLowerCase(), sizeUsdt);
      }
    } else {
      orderId = await placeMarketOrderWithRetry(symbol, side.toLowerCase(), sizeUsdt);
      // Fallback to BingX if KuCoin fails
      if (!orderId && bxOk) {
        console.log(`[FALLBACK] KuCoin failed → trying BingX`);
        orderId = await bxFuturesOrderWithRetry(symbol, side, sizeUsdt, leverage);
      }
    }
    
    if (orderId) {
      console.log(`[SMART EXEC SUCCESS] ${symbol} ${direction} on ${useExchange.toUpperCase()} | OrderId=${orderId}`);
    } else {
      console.error(`[SMART EXEC FAILED] ${symbol} ${direction} - both exchanges failed`);
    }
    
    return { orderId, exchange: useExchange };
  } catch(e) {
    console.error(`[SMART EXEC ERROR] ${symbol}: ${e.message}`);
    return { orderId: null, exchange: null };
  }
}
...
app.post('/api/prefilter',(req,res)=>{
  ...
  // 🔥 REGIME FILTER: Same logic as frontend detectRegime()
  // Top 15 coins h4+h1 trend - SINGLE SOURCE OF TRUTH
  let bulls = 0, bears = 0, regTotal = 0;
  marketCache.slice(0, 15).forEach(c => {
    if (!c) return;
    const t4 = c.h4?.trend, t1 = c.h1?.trend, tm = c.m15?.trend;
    regTotal++;
    if (t4 === 'UP' && (t1 === 'UP' || tm === 'UP')) bulls++;
    else if (t4 === 'DOWN' && (t1 === 'DOWN' || tm === 'DOWN')) bears++;
  });
  const marketRegime = regTotal > 0
    ? (bulls/regTotal >= 0.25 ? 'BULL' : bears/regTotal >= 0.25 ? 'BEAR' : 'SIDE')
    : 'SIDE';
  
  // Filter regime-conflicting signals
  const regimeFiltered = results.filter(r => {
    if (marketRegime === 'BEAR' && r.direction === 'LONG') {
      console.log(`[REGIME BLOCK] ${r.symbol} LONG blocked - BEAR market`);
      return false;
    }
    if (marketRegime === 'BULL' && r.direction === 'SHORT') {
      console.log(`[REGIME BLOCK] ${r.symbol} SHORT blocked - BULL market`);
      return false;
    }
    return true;
  });
  
  console.log(`[REGIME] Market=${marketRegime} | Before=${results.length} After=${regimeFiltered.length}`);
  
  res.json({ok:true, candidates:regimeFiltered, total:regimeFiltered.length, regime:marketRegime});
});

// ── ANALYZE ROUTE ─────────────────────────────────────────────────
let lastAnalyzeTime=0;
// ── LEARNING ENGINE: Query similar past trades ──────────────────────
async function getSimilarTrades(symbol, direction, limit=5) {
  if (!process.env.TURSO_DATABASE_URL) return [];
  try {
    const trades = await db.execute({
      sql: `SELECT symbol, signal as direction, confidence, pnl_pct, outcome 
            FROM signals
            WHERE symbol=? AND signal=? AND outcome IN ('WIN','LOSS')
            ORDER BY id DESC LIMIT ?`,
      args: [symbol, direction, limit]
    });
    return trades.rows || [];
  } catch(e) {
    console.error('[LEARNING] Similar trades query failed:', e.message);
    return [];
  }
}

// ── DYNAMIC KELLY: Symbol + Direction specific (REAL) ──────────────────
async function calculateDynamicKelly(symbol, direction) {
  if (!process.env.TURSO_DATABASE_URL) return 1.0;
  try {
    // FIXED: Symbol + Direction specific (NOT global!)
    const trades = await db.execute({
      sql: `SELECT outcome, pnl_pct FROM signals 
            WHERE symbol=? AND signal=? AND outcome IN ('WIN','LOSS') 
            ORDER BY id DESC LIMIT 20`,
      args: [symbol, direction || '']
    });
    if (!trades.rows || trades.rows.length < 3) return 1.0; // Not enough data
    const wins = trades.rows.filter(t => t.outcome === 'WIN').length;
    const losses = trades.rows.filter(t => t.outcome === 'LOSS').length;
    const winRate = wins / trades.rows.length;
    const avgWin = trades.rows.filter(t=>t.outcome==='WIN').reduce((s,t)=>s+Math.abs(t.pnl_pct||1),0) / (wins||1);
    const avgLoss = trades.rows.filter(t=>t.outcome==='LOSS').reduce((s,t)=>s+Math.abs(t.pnl_pct||1),0) / (losses||1);
    // Real Kelly formula: W - (1-W) / (avgWin/avgLoss)
    const kellyRaw = winRate - (1 - winRate) / Math.max(avgWin / Math.max(avgLoss, 0.1), 0.1);
    let kellyMultiplier = Math.max(0.3, Math.min(2.5, kellyRaw * 0.5)); // Half-Kelly safe
    console.log(`[KELLY] ${symbol} ${direction}: WR=${(winRate*100).toFixed(0)}% avgW=${avgWin.toFixed(2)} avgL=${avgLoss.toFixed(2)} → ${kellyMultiplier.toFixed(2)}x`);
    return kellyMultiplier;
  } catch(e) {
    console.error('[KELLY] Error:', e.message);
    return 1.0;
  }
}

// ── RECENT WIN RATE (for Claude context) ──────────────────────────────
async function getRecentStats() {
  if (!process.env.TURSO_DATABASE_URL) return { wins: 0, losses: 0, winRate: 0 };
  try {
    const stats = await db.execute({
      sql: `SELECT outcome, COUNT(*) as cnt FROM signals 
            WHERE outcome IN ('WIN','LOSS') 
            GROUP BY outcome`,
      args: []
    });
...
// Self-tuning parameters — adjust automatically based on recent performance
let autoParams = {
  scoreThreshold:      90,    // preFilter min score (was hardcoded 90)
  confidenceThreshold: 7.0,   // Claude min confidence (was hardcoded 7.0)
  aggressiveMode:      false, // aggressive preFilter mode
  lastOptimized:       null,
  lastWR:              null,
};
...
async function getPerformanceStats() {
  ...
  try {
    const rows = await db.execute({
      sql: `SELECT pnl_pct, outcome FROM signals
            WHERE outcome IN ('WIN','LOSS')
            AND pnl_pct IS NOT NULL
    ...
    if (trades.length < 10) return null; // need minimum 10 trades
    const wins   = trades.filter(t => t.outcome === 'WIN');
    const losses = trades.filter(t => t.outcome === 'LOSS');
    const winRate = wins.length / trades.length;
    const avgWin  = wins.length   ? wins.reduce((s,t)=>s+Math.abs(t.pnl_pct||0),0)   / wins.length   : 0;
    const avgLoss = losses.length ? losses.reduce((s,t)=>s+Math.abs(t.pnl_pct||0),0) / losses.length : 0;
    return { winRate, avgWin, avgLoss, total: trades.length };
  } catch(e) { return null; }
}
...
async function autoOptimize() {
  try {
    ...
    if (!stats) { console.log('[AUTO OPT] Not enough data (need 10+ trades)'); return; }

    const { winRate, avgWin, avgLoss } = stats;
    const wr = (winRate * 100).toFixed(1);
    console.log(`[AUTO OPT] Analyzing: WR=${wr}% avgWin=${avgWin.toFixed(2)}% avgLoss=${avgLoss.toFixed(2)}%`);

    const prev = { ...autoParams };

    // 🔴 Losing (WR < 40%) → be conservative
    if (winRate < 0.40) {
      autoParams.scoreThreshold      = Math.min(autoParams.scoreThreshold + 3, 105);
      autoParams.confidenceThreshold = Math.min(autoParams.confidenceThreshold + 0.3, 8.5);
      autoParams.aggressiveMode      = false;
      console.log(`[AUTO OPT] 🔴 WR=${wr}% LOW → stricter filters`);
    }
    // 🟢 Winning (WR > 55%) → be aggressive
    else if (winRate > 0.55) {
      autoParams.scoreThreshold      = Math.max(autoParams.scoreThreshold - 2, 75);
      autoParams.confidenceThreshold = Math.max(autoParams.confidenceThreshold - 0.2, 6.5);
      autoParams.aggressiveMode      = true;
      console.log(`[AUTO OPT] 🟢 WR=${wr}% HIGH → looser filters`);
    }
    // 🟡 Neutral (40–55%) → fine tune based on avg win vs loss
    else {
      if (avgWin > avgLoss * 1.5) {
        autoParams.scoreThreshold = Math.max(autoParams.scoreThreshold - 1, 80);
        console.log(`[AUTO OPT] 🟡 WR=${wr}% neutral but RR good → slight looser`);
      } else {
        autoParams.scoreThreshold = Math.min(autoParams.scoreThreshold + 1, 100);
        console.log(`[AUTO OPT] 🟡 WR=${wr}% neutral RR weak → slight stricter`);
      }
    }

    // 🔒 SAFETY LIMITS — never go out of bounds
    autoParams.scoreThreshold      = Math.max(75,  Math.min(105, autoParams.scoreThreshold));
    autoParams.confidenceThreshold = Math.max(6.5, Math.min(8.5, autoParams.confidenceThreshold));
    autoParams.lastOptimized       = new Date().toISOString();
    autoParams.lastWR              = wr;

    // Log changes
    if (prev.scoreThreshold !== autoParams.scoreThreshold || prev.confidenceThreshold !== autoParams.confidenceThreshold) {
      console.log(`[AUTO OPT] ✅ PARAMS UPDATED: score ${prev.scoreThreshold}→${autoParams.scoreThreshold} | conf ${prev.confidenceThreshold}→${autoParams.confidenceThreshold} | aggressive=${autoParams.aggressiveMode}`);
    } else {
      console.log(`[AUTO OPT] No change needed: score=${autoParams.scoreThreshold} conf=${autoParams.confidenceThreshold}`);
    }
  } catch(e) {
    console.error('[AUTO OPT] Error:', e.message);
  }
}
...
async function updateLossCounter() {
  ...
  try {
    const recent = await db.execute({
      sql: `SELECT outcome FROM signals WHERE outcome IN ('WIN','LOSS') ORDER BY id DESC LIMIT 3`,
      args: []
    });
    ...
    if (consecutiveLosses >= 3) {
      isPaused = true;
      console.log(`⚠️ [PROTECTION] 3 consecutive losses! Auto-pausing trading for 1 hour.`);
      setTimeout(() => { isPaused = false; }, 60*60*1000); // Pause 1 hour
    } else {
      isPaused = false;
    }
    
    console.log(`[LOSS COUNTER] ${consecutiveLosses} consecutive losses | Paused: ${isPaused}`);
  } catch(e) {
    console.error('[LOSS COUNTER] Error:', e.message);
  }
}

// ── SYSTEM LEVEL SCORING: Claude Confidence × Historical Accuracy ──────
async function calculateSystemScore(symbol, direction, claudeConfidence) {
  if (!process.env.TURSO_DATABASE_URL) {
    console.log(`[SYSTEM SCORE] SKIP: No DB`);
    return claudeConfidence;
  }
  
  try {
    // Get similar trades for this symbol + direction from signals table
    const similar = await db.execute({
      sql: `SELECT outcome FROM signals 
            WHERE symbol=? AND signal=? AND outcome IN ('WIN','LOSS')
            ORDER BY id DESC LIMIT 10`,
      args: [symbol, direction]
    });
    
    const trades = similar.rows || [];
    console.log(`[SYSTEM SCORE] ${symbol} ${direction}: Found ${trades.length} similar trades`);
    
    if (trades.length < 3) {
      console.log(`[SYSTEM SCORE] Not enough history (${trades.length}<3), using Claude ${claudeConfidence}`);
      return claudeConfidence;
    }
    
    // Calculate historical accuracy for this specific setup
    const wins = trades.filter(t => t.outcome === 'WIN').length;
    const historicalAccuracy = wins / trades.length;
    
    // System score = Claude × Historical
    const systemScore = claudeConfidence * (0.5 + historicalAccuracy * 0.5);
    
    console.log(`[SYSTEM SCORE] ${symbol} ${direction}: Claude=${claudeConfidence.toFixed(1)} × History=${(historicalAccuracy*100).toFixed(0)}% = ${systemScore.toFixed(1)}`);
    
    return Math.max(4.0, Math.min(10.0, systemScore));
  } catch(e) {
    console.error(`[SYSTEM SCORE ERROR] ${symbol} ${direction}: ${e.message}`);
    return claudeConfidence;
  }
}

// ── FEEDBACK INJECTION: Compare to similar past trades ────────────────
async function generateFeedbackContext(symbol, direction, entry_price) {
  if (!process.env.TURSO_DATABASE_URL) {
    console.log(`[FEEDBACK] SKIP: No DB`);
    return '';
  }
  
  try {
    const similar = await db.execute({
      sql: `SELECT pnl_pct, outcome, confidence 
            FROM signals 
            WHERE symbol=? AND signal=? AND outcome IN ('WIN','LOSS')
    ...
    const trades = similar.rows || [];
    console.log(`[FEEDBACK] ${symbol} ${direction}: Found ${trades.length} past trades`);
    
    if (trades.length === 0) {
      console.log(`[FEEDBACK] No history for ${symbol} ${direction}`);
      return '';
    }
    ...
    let feedback = `\n📊 FEEDBACK: ${symbol} ${direction} setup history:\n`;
    
    if (wins.length > 0) {
      const avgWinConf = (wins.reduce((sum, t) => sum + t.confidence, 0) / wins.length).toFixed(1);
      const avgWinPnL = (wins.reduce((sum, t) => sum + t.pnl_pct, 0) / wins.length).toFixed(2);
      feedback += `✅ Wins: ${wins.length}/${trades.length} | Avg confidence: ${avgWinConf} | Avg PnL: ${avgWinPnL}%\n`;
    }
    
    if (losses.length > 0) {
      const avgLossConf = (losses.reduce((sum, t) => sum + t.confidence, 0) / losses.length).toFixed(1);
      const avgLossPnL = (losses.reduce((sum, t) => sum + t.pnl_pct, 0) / losses.length).toFixed(2);
      feedback += `❌ Losses: ${losses.length}/${trades.length} | Avg confidence: ${avgLossConf} | Avg Loss: ${avgLossPnL}%\n`;
    }
    
    feedback += `→ Use past patterns: high confidence setups on THIS coin had better outcomes\n`;
    
    console.log(`[FEEDBACK] Generated for ${symbol} ${direction}: ${feedback.length} chars`);
    
    return feedback;
  } catch(e) {
    console.error(`[FEEDBACK ERROR] ${symbol} ${direction}: ${e.message}`);
    return '';
  }
}

// ── PER-SETUP KELLY: Individual sizing based on setup success rate ──────
async function calculateSetupKelly(symbol, direction) {
  if (!process.env.TURSO_DATABASE_URL) {
    console.log(`[SETUP KELLY] SKIP: No DB`);
    return 1.0;
  }
  
  try {
    const setupTrades = await db.execute({
      sql: `SELECT outcome, pnl_pct FROM signals 
            WHERE symbol=? AND signal=? AND outcome IN ('WIN','LOSS')
    ...
    const trades = setupTrades.rows || [];
    console.log(`[SETUP KELLY] ${symbol} ${direction}: Found ${trades.length} trades`);
    
    if (trades.length < 3) {
      console.log(`[SETUP KELLY] Not enough data (${trades.length}<3), using 1.0x`);
      return 1.0;
    }
    ...
    let setupKelly = 0.3 + (winRate * 1.2);
    setupKelly = Math.max(0.3, Math.min(1.5, setupKelly));
    
    console.log(`[SETUP KELLY] ${symbol} ${direction}: ${(winRate*100).toFixed(0)}% WR (${wins}/${trades.length}) → ${setupKelly.toFixed(2)}x size`);
    
    return setupKelly;
  } catch(e) {
    console.error(`[SETUP KELLY ERROR] ${symbol} ${direction}: ${e.message}`);
    return 1.0;
  }
}

// ── REGIME AI: Detect trend/range/volatile ───────────────────────────
function detectRegime(candles) {
  try {
    if (!candles || candles.length < 30) {
      console.log(`[REGIME] Not enough candles (${candles?.length || 0}), defaulting to UNKNOWN`);
      return 'UNKNOWN';
    }

    const closes = candles.map(c => parseFloat(c.close) || 0).filter(x => x > 0);
    ...
    const vol = Math.sqrt(Math.max(0, variance)); // Safe sqrt

    // Trend slope
    const slope = closes[closes.length-1] - closes[0];
    const slopePercent = Math.abs(slope) / closes[0];

    let regime = 'UNKNOWN';
    if (vol > 0.03) regime = 'VOLATILE';
    else if (slopePercent < 0.01) regime = 'RANGE';
    else regime = slope > 0 ? 'TREND_UP' : 'TREND_DOWN';

    console.log(`[REGIME] Vol=${(vol*100).toFixed(2)}% | Slope=${(slopePercent*100).toFixed(2)}% → ${regime}`);
    return regime;
  } catch(e) {
...
// ── STRATEGY PICKER: Pick strategy based on regime ───────────────────
function pickStrategy(regime) {
  try {
    const strategies = {
      'TREND_UP': 'TREND_LONG',
      'TREND_DOWN': 'TREND_SHORT',
      'RANGE': 'MEAN_REVERSION',
      'VOLATILE': 'LOW_RISK',
      'UNKNOWN': 'DEFAULT'
    };
    return strategies[regime] || 'DEFAULT';
  } catch(e) {
...
// ── LOSS STREAK PROTECTION: Stop trading after N losses ──────────────
async function checkLossStreak(symbol) {
  try {
    ...
    if (streak >= 3) {
      console.log(`[RISK] ${symbol} paused - ${streak} consecutive losses`);
      return true; // Block new trades
    }

    return false;
  } catch(e) {
    console.error(`[LOSS STREAK ERROR]: ${e.message}`);
    return false; // Safe default - allow trade
  }
}

// ── ADAPTIVE TP/SL: Learn from past wins ──────────────────────────────
async function getAdaptiveTPSL(symbol, direction) {
  try {
    if (!process.env.TURSO_DATABASE_URL) {
      return { tp: 2.0, sl: 1.0 }; // Default
    }

    const res = await db.execute({
      sql: `SELECT pnl_pct FROM signals 
            WHERE symbol=? AND signal=? AND outcome='WIN'
            ORDER BY id DESC LIMIT 20`,
      args: [symbol, direction]
    });
    ...
    if (wins.length < 5) {
      console.log(`[TP/SL] ${symbol} ${direction}: Not enough wins (${wins.length}), using defaults`);
      return { tp: 2.0, sl: 1.0 };
    }

    const avgPnL = wins.reduce((s,t) => s + Math.abs(t.pnl_pct || 0), 0) / wins.length;
    
    // TP = average win * 1.2 (target 20% better)
    // SL = TP / 2.5 (risk/reward ratio)
    let tp = Math.min(avgPnL * 1.2, 5.0); // Cap at 5%
    let sl = Math.max(tp / 2.5, 0.5); // Min 0.5%

    tp = Math.max(tp, 1.5); // Minimum 1.5% TP
    sl = Math.max(sl, 0.8); // Minimum 0.8% SL

    console.log(`[TP/SL AI] ${symbol} ${direction}: ${wins.length} wins avg=${avgPnL.toFixed(2)}% → TP=${tp.toFixed(2)}% SL=${sl.toFixed(2)}%`);
    
    return { tp, sl };
  } catch(e) {
    console.error(`[TP/SL ERROR]: ${e.message}`);
    return { tp: 2.0, sl: 1.0 }; // Safe default
  }
}

// ── REGIME BOOST: Adjust confidence based on market regime ───────────
function applyRegimeBoost(score, regime, direction) {
  try {
    let adjusted = parseFloat(score) || 7.0;

    // Boost aligned trades
    if (regime === 'TREND_UP' && direction === 'LONG') adjusted += 0.8;
    else if (regime === 'TREND_DOWN' && direction === 'SHORT') adjusted += 0.8;

    // Reduce risky trades
    if (regime === 'RANGE') adjusted -= 0.5;
    if (regime === 'VOLATILE') adjusted -= 1.0;

    // Bounds
    adjusted = Math.max(4.0, Math.min(10.0, adjusted));

    if (adjusted !== score) {
      console.log(`[REGIME BOOST] ${regime} ${direction}: ${score.toFixed(1)} → ${adjusted.toFixed(1)}`);
    }

    return adjusted;
  } catch(e) {
    console.error(`[REGIME BOOST ERROR]: ${e.message}`);
    return score; // Return original if error
  }
}

// ── INSTITUTIONAL MODULES ────────────────────────────────────────────

// Correlation between two price arrays
function correlation(a, b) {
  try {
    ...
    console.error(`[CORR ERROR]: ${e.message}`);
    return 0; // Safe default
  }
}

// How correlated is target coin with open positions?
function correlationPenalty(target, openList) {
  try {
    if (!openList || !openList.length || !target || !target.length) return 0;
    let max = 0;
    for (const r of openList) {
      const c = Math.abs(correlation(target, r));
      if (c > max) max = c;
    }
    if (max > 0.85) return 1;   // Nearly identical → block
    if (max > 0.70) return 0.7; // High correlation → reduce confidence
    if (max > 0.50) return 0.4; // Medium correlation → slight reduce
    return 0;
  } catch(e) {
    console.error(`[CORR PENALTY ERROR]: ${e.message}`);
    return 0; // Safe default - no penalty
  }
}

// Position sizing based on equity + stop loss + kelly
function positionSize({ equity, stopPct, kellyMult=1 }) {
  try {
    if (!equity || !stopPct) return 10; // Default $10
    const riskDollar = equity * 0.01 * kellyMult; // 1% risk per trade
    const size = riskDollar / Math.max(stopPct/100, 0.001);
    return Math.max(5, Math.min(size, equity * 0.20)); // Min $5, Max 20% equity
  } catch(e) {
...
// Drawdown guard - stop trading if total loss > limit%
async function checkDrawdown(limit=5) {
  try {
    if (!process.env.TURSO_DATABASE_URL) return false;
    const r = await db.execute({
      sql: `SELECT pnl_pct FROM signals WHERE outcome IN ('WIN','LOSS') ORDER BY id DESC LIMIT 50`,
      args: []
    });
    const arr = (r.rows || []).map(x => x.pnl_pct || 0);
    if (arr.length < 3) return false; // Not enough data
    const sum = arr.reduce((a,b) => a+b, 0);
    if (sum < -limit) {
      console.log(`[DD STOP] Total PnL=${sum.toFixed(2)}% — below limit ${limit}% — trading paused`);
      return true; // Block trading
    }
    return false;
  } catch(e) {
    console.error(`[DD ERROR]: ${e.message}`);
    return false; // Safe default - allow trading
  }
}
...
app.post('/api/analyze',async(req,res)=>{
  ...
  // PHASE 2: Check loss protection auto-pause
  await updateLossCounter();
  if(isPaused) return res.status(429).json({ok:false,error:'Trading paused: 3 consecutive losses - cooling down 1 hour'});
  const now=Date.now();
  ...
  try {
    ...
    // LEARNING ENGINE: Get historical context + dynamic Kelly
    let historicalContext = '';
    let dynamicKelly = 2.0;
    
    if(promptSymbol !== 'N/A') {
      // Get similar PAST trades (wins and losses)
      const similarLongs = await getSimilarTrades(promptSymbol, 'LONG', 5);
      const similarShorts = await getSimilarTrades(promptSymbol, 'SHORT', 5);
      const recentStats = await getRecentStats();
      dynamicKelly = await calculateDynamicKelly(promptSymbol, 'LONG'); // Default LONG for context
      
      // FEEDBACK INJECTION: Past trade patterns
      const feedbackLong = await generateFeedbackContext(promptSymbol, 'LONG', 0);
      const feedbackShort = await generateFeedbackContext(promptSymbol, 'SHORT', 0);
      
      // Build context string
      let contextStr = `\n🧠 LEARNING CONTEXT:\n`;
      contextStr += `Total history: ${recentStats.total} trades (${recentStats.wins}W ${recentStats.losses}L) = ${recentStats.winRate}% WR\n`;
      
      if(similarLongs.length > 0) {
        const longWins = similarLongs.filter(t => t.outcome === 'WIN').length;
        const longWR = ((longWins/similarLongs.length)*100).toFixed(0);
        const longFlag = longWins === 0 ? '🚫 AVOID — never won LONG' : longWR >= 65 ? '✅ STRONG' : longWR < 35 ? '⚠️ POOR' : '';
        contextStr += `${promptSymbol} LONG history: ${longWins}/${similarLongs.length} wins (${longWR}% WR) ${longFlag}\n`;
      }
      if(similarShorts.length > 0) {
        const shortWins = similarShorts.filter(t => t.outcome === 'WIN').length;
        const shortWR = ((shortWins/similarShorts.length)*100).toFixed(0);
        const shortFlag = shortWins === 0 ? '🚫 AVOID — never won SHORT' : shortWR >= 65 ? '✅ STRONG' : shortWR < 35 ? '⚠️ POOR' : '';
        contextStr += `${promptSymbol} SHORT history: ${shortWins}/${similarShorts.length} wins (${shortWR}% WR) ${shortFlag}\n`;
      }
      // Per-coin WR direct instruction for Claude
      if(similarLongs.length > 0 || similarShorts.length > 0) {
        const totalTrades = similarLongs.length + similarShorts.length;
        const totalWins = similarLongs.filter(t=>t.outcome==='WIN').length + similarShorts.filter(t=>t.outcome==='WIN').length;
        const overallWR = totalTrades > 0 ? ((totalWins/totalTrades)*100).toFixed(0) : 0;
        if (totalWins === 0) {
          contextStr += `⚠️ CRITICAL: ${promptSymbol} has ZERO wins in ${totalTrades} trades. Max confidence = 5.0. Only trade if 3+ perfect indicators align.\n`;
        } else if (parseFloat(overallWR) >= 65) {
          contextStr += `✅ ${promptSymbol} is a HIGH PERFORMER (${overallWR}% WR). Add +0.5 confidence if setup aligns.\n`;
        }
      }
      contextStr += `Dynamic Kelly: ${dynamicKelly.toFixed(2)}x (adaptive to win rate)\n`;
      contextStr += feedbackLong + feedbackShort;
      
      historicalContext = contextStr;
    }

    const r=await axios.post('https://api.anthropic.com/v1/messages',{
      model:model||'claude-sonnet-4-20250514',
      max_tokens:2500,
      system:`You are NEXUS PRO, an elite crypto trading agent. Capital preservation first, then profit.
Coins passed a strict pre-filter with MACD, BB, StochRSI, Supertrend, VWAP indicators.
Your job: pick only the BEST 1-2 setups. Quality > Quantity. Missing a trade is fine. Losing is not.

CONFLUENCE SCORING (Smart - accept quality 2+ indicator setups):
- 3+ indicators align → HIGH confidence (8.0-10.0) → STRONG ENTRY
- 2 indicators align → MEDIUM confidence (6.5-7.9) → GOOD ENTRY  
- 1 indicator only → LOW confidence (< 6.0) → SKIP

ENTRY RULES:
1. Minimum 2 indicators must align (3+ preferred)
   - 3-timeframe + Volume + RSI = perfect (conf 8-10)
   - Trend + Volume OR Trend + RSI = acceptable (conf 6.5-7.9)
   - Only 1 signal = SKIP (conf < 6)
2. Volume: volRatio >= 1.3 for HIGH, >= 1.0 for MEDIUM confidence
3. RSI not extreme: LONG < 65, SHORT > 35
4. MACD direction matches trade
5. SIDE regime: need 3+ indicators only
6. SL: 1.5-3.5% | TP: min 2.5x SL
7. Coins with btWR = historically tested, prefer them
8. Return [] if nothing meets criteria — waiting is fine

⚡ REGIME STRICT RULES (LEARNING AGENT):
- BEAR regime: ONLY SHORT trades. NO LONG (high risk in downtrend)
- BULL regime: ONLY LONG trades. NO SHORT (opposing uptrend)
- SIDE regime: Both OK (but need 3+ indicators)
- If current regime conflicts with trade direction → SKIP or reduce confidence

Return ONLY raw JSON. No markdown.
FORMAT: {"market_read":"...","signals":[{"symbol":"...","direction":"LONG/SHORT","confidence":7.0,"sl_pct":2.0,"tp_pct":5.0,"reason":"2 indicators: trend+volume aligned","trade_type":"spot/futures","leverage":1}],"skip_reason":"..."}
${memory}${historicalContext}`,
    ...
    if(process.env.TURSO_DATABASE_URL&&Array.isArray(parsed.signals)&&parsed.signals.length>0){
      ...
      // 🔥 DRAWDOWN GUARD: Check ONCE before all signals
      const ddHit = await checkDrawdown(5);
      if(ddHit) {
        console.log(`[DD STOP] All signals blocked - drawdown limit hit`);
        return res.json({ok:true, result:{market_read:'DRAWDOWN STOP', signals:[], skip_reason:'Drawdown limit - trading paused'}});
      }

      for(const sig of parsed.signals){
        if(!['LONG','SHORT'].includes(sig.direction)||!sig.symbol||sig.symbol==='N/A') continue;
        try {
          // 🔥 LOSS STREAK: Per-symbol protection
          const hasLossStreak = await checkLossStreak(sig.symbol);
          if(hasLossStreak) {
            console.log(`[BLOCK] ${sig.symbol} ${sig.direction} - Loss streak protection`);
            continue;
          }
          
          // 🔥 REGIME: Market context detection
          // FIX: Use market-wide regime (same as auto cycle) — more reliable than per-coin candles
          // Per-coin candles often missing for small coins → UNKNOWN → wrong strategy
          const coin = marketCache.find(c=>c.symbol===sig.symbol);
          const btcCoin = marketCache.find(c=>c.symbol==='BTC');

          // Method 1: Market-wide regime from top 15 coins (most reliable)
          let regime = 'SIDE';
          try {
            let rt=0, rb=0, rbe=0;
            marketCache.slice(0,15).forEach(c=>{
              ...
              const t4=c.h4?.trend, t1=c.h1?.trend, tm=c.m15?.trend;
              rt++;
              if(t4==='UP'&&(t1==='UP'||tm==='UP')) rb++;
              else if(t4==='DOWN'&&(t1==='DOWN'||tm==='DOWN')) rbe++;
            });
            if(rt>0) regime = rb/rt>=0.25?'BULL':rbe/rt>=0.25?'BEAR':'SIDE';
          } catch(re) {
            // Method 2: BTC candle fallback
            const btcCandles = btcCoin?.m15?.candles?.length >= 30 ? btcCoin.m15.candles : [];
            // Method 3: Coin candle fallback
            const coinCandles = coin?.m15?.candles?.length >= 30 ? coin.m15.candles : [];
            const fallbackCandles = btcCandles.length ? btcCandles : coinCandles;
            if(fallbackCandles.length >= 30) regime = detectRegime(fallbackCandles);
          }
          const strategy = pickStrategy(regime);
          
          // 🔥 CORRELATION: Check vs open positions (safe - won't crash)
          let corrPenalty = 0;
          try {
            const targetReturns = (coin?.m15?.candles || []).map(c => parseFloat(c.close) || 0);
            const openTrades = await db.execute({sql:`SELECT symbol FROM active_trades WHERE status='OPEN' LIMIT 5`,args:[]});
            const openReturnsArr = [];
            for (const ot of (openTrades.rows||[])) {
              const otCoin = marketCache.find(c=>c.symbol===ot.symbol);
              if (otCoin?.m15?.candles?.length) openReturnsArr.push(otCoin.m15.candles.map(c=>parseFloat(c.close)||0));
            }
            corrPenalty = correlationPenalty(targetReturns, openReturnsArr);
            if (corrPenalty === 1) {
              console.log(`[CORR BLOCK] ${sig.symbol} - Too correlated with open trades`);
              continue;
            }
            if (corrPenalty > 0) console.log(`[CORR] ${sig.symbol}: penalty=${corrPenalty.toFixed(2)}`);
          } catch(corrErr) { /* Safe - no penalty on error */ }
          ...
          // 🔥 NEW: Adaptive TP/SL (learn from past wins)
          let tpPrice = null;
          let tp_pct = sig.tp_pct || 2.0;
          let sl_pct = sig.sl_pct || 1.0;
          
          try {
            const { tp: adaptiveTP, sl: adaptiveSL } = await getAdaptiveTPSL(sig.symbol, sig.direction);
            tp_pct = adaptiveTP;
            sl_pct = adaptiveSL;
            console.log(`[ADAPT] ${sig.symbol} ${sig.direction}: Using adaptive TP=${tp_pct.toFixed(2)}% SL=${sl_pct.toFixed(2)}%`);
          } catch(e) {
            console.log(`[ADAPT] Fallback to default TP/SL`);
          }
          
          tpPrice = price && tp_pct ? parseFloat((price*(sig.direction==='LONG'?1+tp_pct/100:1-tp_pct/100)).toFixed(6)) : null;
          ...
          if(orderMode === 'futures') {
            finalTradeType = 'futures';
          } else if(orderMode === 'spot') {
            finalTradeType = 'spot';
          } else {
            finalTradeType = sig.trade_type || sigTradeDecision.type || 'spot';
          }
          // EXCHANGE ROUTING: Hash-based fair distribution between KuCoin + BingX
          let finalExchange = 'kucoin'; // default
          try {
            const kuOk = !!(process.env.KUCOIN_API_KEY && process.env.KUCOIN_API_SECRET && process.env.KUCOIN_PASSPHRASE);
            const bxOk = !!(BINGX_API_KEY && BINGX_SECRET);
            
            if (kuOk && bxOk) {
              // Both available - use symbol hash for fair split
              if (global.bxOnlyCoins?.has(sig.symbol)) {
                finalExchange = 'bingx'; // BingX-only coin
              } else {
                // Round-robin alternating (fair split)
                global.exchCounter = (global.exchCounter || 0) + 1;
                finalExchange = global.exchCounter % 2 === 0 ? 'bingx' : 'kucoin';
              }
            } else if (bxOk) {
              finalExchange = 'bingx';
            } else {
              finalExchange = 'kucoin';
            }
            console.log(`[EXCHANGE] ${sig.symbol} → ${finalExchange.toUpperCase()}`);
          } catch(e) { finalExchange = 'kucoin'; }
          ...
          // 🔥 LEARNING: Feedback-based confidence adjustment
          let finalConfidence = sig.confidence || 7.0;
          
          try {
            // Get similar past trades for THIS symbol + direction
            const similar = await db.execute({
              sql: `SELECT outcome FROM signals 
                    WHERE symbol=? AND signal=? AND outcome IN ('WIN','LOSS')
                    ORDER BY id DESC LIMIT 10`,
            ...
            const similarTrades = similar.rows || [];
            ...
            if (similarTrades.length >= 3) {
              ...
              finalConfidence = Math.max(4.0, Math.min(10.0, finalConfidence));
              console.log(`[LEARNING] ${sig.symbol} ${sig.direction}: WR=${(winRate*100).toFixed(0)}% (${wins}/${similarTrades.length})`);
              console.log(`[SYSTEM SCORE] Claude=${sig.confidence} → System=${finalConfidence.toFixed(1)}`);
            } else {
              console.log(`[LEARNING] ${sig.symbol} ${sig.direction}: No history (${similarTrades.length} trades) - using Claude score`);
            }
          } catch(feedbackErr) {
            console.error(`[FEEDBACK ERROR] ${sig.symbol} ${sig.direction}:`, feedbackErr.message);
          }
          
          // 🔥 NEW: Apply regime boost (market-aware adjustment)
          finalConfidence = applyRegimeBoost(finalConfidence, regime, sig.direction);
          
          // 🔥 CORRELATION PENALTY: Reduce confidence if correlated with open trades
          if (corrPenalty > 0) {
            finalConfidence = Math.max(4.0, finalConfidence - (corrPenalty * 1.2));
            console.log(`[CORR APPLIED] ${sig.symbol}: confidence reduced to ${finalConfidence.toFixed(1)}`);
          }
          
          // Final bounds
          finalConfidence = Math.max(4.0, Math.min(10.0, finalConfidence));
          console.log(`[FINAL AI] ${sig.symbol} ${sig.direction}: ${sig.confidence} → ${finalConfidence.toFixed(1)} | Regime=${regime} | Strategy=${strategy}`);
          
          // PER-SETUP KELLY: Individual setup sizing
          let setupKelly = 1.0;
          try {
            const setupTrades = await db.execute({
              sql: `SELECT outcome FROM signals 
            ...
            if (trades.length >= 3) {
              ...
              const setupWR = setupWins / trades.length;
              setupKelly = 0.3 + (setupWR * 1.2);
              setupKelly = Math.max(0.3, Math.min(1.5, setupKelly));
              console.log(`[SETUP KELLY] ${sig.symbol} ${sig.direction}: ${(setupWR*100).toFixed(0)}% → ${setupKelly.toFixed(2)}x`);
            }
          } catch(kellyErr) {
            console.error(`[SETUP KELLY ERROR]:`, kellyErr.message);
          }
          
          // 🔥 POSITION SIZE: Kelly-adjusted smart sizing
          let finalSize = 10; // Default $10
          try {
            const portfolioRes = await db.execute({sql:`SELECT cash FROM portfolio_state ORDER BY id DESC LIMIT 1`,args:[]});
            const equity = portfolioRes.rows[0]?.cash || 100;
            finalSize = positionSize({ equity, stopPct: sl_pct, kellyMult: setupKelly });
            console.log(`[SIZE] ${sig.symbol}: equity=$${equity} stop=${sl_pct.toFixed(2)}% kelly=${setupKelly.toFixed(2)}x → $${finalSize.toFixed(2)}`);
          } catch(sizeErr) { /* Use default $10 */ }
          
          await db.execute({
            sql:`INSERT INTO signals (symbol,signal,entry,tp,sl,confidence,reasoning,trade_type,exchange) VALUES (?,?,?,?,?,?,?,?,?)`,
            args:[sig.symbol,sig.direction,price,tpPrice,slPrice,finalConfidence||null,sig.reason||null,
                  finalTradeType, finalExchange]
          });
          console.log(`Signal saved: ${sig.symbol} ${sig.direction} | Final=${finalConfidence.toFixed(1)} | Kelly=${setupKelly.toFixed(2)}x | Size=$${finalSize.toFixed(2)} | Regime=${regime}`);
...
// ── FUNDING RATE CHECK ───────────────────────────────────────────
async function checkFundingRate(symbol, direction, useBingX=true) {
  try {
    let rate = 0;
    if(useBingX) {
      // BingX funding rate API
      ...
      if(String(r.data.code) !== '0') return { ok:true };
      rate = parseFloat(r.data.data?.lastFundingRate || 0) * 100;
    } else {
      // KuCoin futures funding rate API
      const r = await axios.get(`https://api-futures.kucoin.com/api/v1/funding-rate/${symbol}USDTM/current`, {
        timeout: 5000
      });
      if(r.data.code !== '200000') return { ok:true };
      rate = parseFloat(r.data.data?.value || 0) * 100;
    }
    const MAX = 0.15;
...
app.post('/api/portfolio/save', async(req,res) => {
  ...
  try {
    const {cash} = req.body;
    if(cash == null) return res.status(400).json({ok:false,error:'cash required'});
    // Upsert — always keep only 1 row
    await db.execute({
      sql: `INSERT INTO portfolio_state (id, cash, updated_at) VALUES (1, ?, CURRENT_TIMESTAMP)
            ON CONFLICT(id) DO UPDATE SET cash=excluded.cash, updated_at=CURRENT_TIMESTAMP`,
      args: [parseFloat(cash)]
    });
    res.json({ok:true});
  } catch(e) { res.status(500).json({ok:false,error:e.message}); }
});

app.get('/api/portfolio/load', async(req,res) => {
  if(!process.env.TURSO_DATABASE_URL) return res.json({ok:true, cash:null});
  try {
    const row = await db.execute(`SELECT cash FROM portfolio_state WHERE id=1`);
    const cash = row.rows.length > 0 ? parseFloat(row.rows[0].cash) : null;
    res.json({ok:true, cash});
  } catch(e) { res.json({ok:true, cash:null}); }
});
...
app.post('/api/order',async(req,res)=>{
  ...
  try {
    ...
    // Exchange selection — simple aur reliable
    let useBingX = false;
    if(!kuOk && bxOk) {
      useBingX = true; // Sirf BingX configured
    } else if(kuOk && bxOk) {
      // Dono configured:
      // BingX-only coin (KuCoin pe available nahi) → BingX
      // KuCoin-only coin → KuCoin
      // Common coin → alternate karo (har trade pe dono use ho)
      if(global.bxOnlyCoins?.has(symbol)) {
        useBingX = true;
      } else if(global.kuOnlyCoins?.has(symbol)) {
        useBingX = false;
      } else {
        // Common coin — alternating round-robin (fair distribution)
        global.exchCounter = (global.exchCounter || 0) + 1;
        useBingX = global.exchCounter % 2 === 0;
      }
      console.log(`[EXCHANGE] ${symbol}: ${useBingX?'BingX':'KuCoin'} (bxOnly=${global.bxOnlyCoins?.has(symbol)} kuOnly=${global.kuOnlyCoins?.has(symbol)})`);
    }
    ...
    if(isFutures) {
      const fr = await checkFundingRate(symbol, direction, useBingX);
      if(!fr.ok) {
    ...
    if(useBingX) {
      exchange = 'bingx';
      if(isFutures) {
        // BingX Futures with SMART RETRY
        const bxSide = direction==='LONG' ? 'BUY' : 'SELL';
        const r = await bxFuturesOrderWithRetry(symbol, bxSide, sizeUsdt, lev);
      ...
      // SL/TP managed by Guardian for BingX
    } else {
      exchange = 'kucoin';
      if(isFutures) {
        // KuCoin Futures — kuFuturesOrder (margin cross)
        try {
          entryId = await kuFuturesOrder(symbol, side, sizeUsdt, lev);
          console.log(`[KUCOIN FUTURES] ${symbol} ${direction} entry=${entryId}`);
        } catch(kuErr) {
          console.log(`[KUCOIN FUTURES] Failed: ${kuErr.message} — falling back to BingX`);
          exchange = 'bingx';
          const bxSide = direction==='LONG' ? 'BUY' : 'SELL';
          const r = await bxFuturesOrderWithRetry(symbol, bxSide, sizeUsdt, lev);
          entryId = r?.orderId || 'bxf_'+Date.now();
        ...
        // KuCoin Spot with SMART RETRY
        entryId = await placeMarketOrderWithRetry(symbol, side, sizeUsdt);
...
app.post('/api/trades/open', async(req,res) => {
  ...
  try {
    const {signal_id,symbol,direction,entry_price,tp_price,sl_price,sl_pct,tp_pct,
           confidence,size_usdt,reasoning,trailing,tp1_hit,tp2_hit,
    ...
    // ── DUPLICATE GUARD: Same coin + same exchange → block ───────────────
    const dupBlock = await db.execute({
      sql:`SELECT id FROM active_trades WHERE status='OPEN' AND symbol=? AND exchange=? LIMIT 1`,
      args:[symbol, safeExchange]
    });
    if(dupBlock.rows.length > 0){
      console.log(`[TRADE BLOCK] ${symbol} already OPEN on ${safeExchange} — rejected`);
      return res.status(400).json({ok:false, error:`${symbol} already open on ${safeExchange}`, blocked:true});
    }
    ...
    const result = await db.execute({
      ...
      args: [signal_id||null, symbol, direction, entry_price, tp_price||null, finalSLPrice||sl_price||null,
             ...
             trailing?1:0, tp1_hit?1:0, tp2_hit?1:0,
...
app.post('/api/trades/close', async(req,res) => {
  ...
  try {
    ...
    // signals table mein outcome save karo
    const outcome = (pnl_pct != null && pnl_pct >= 0) ? 'WIN' : 'LOSS';
    // First get the trade details
    const tradeForSignal = await db.execute({
      sql: `SELECT symbol, direction, signal_id, confidence FROM active_trades WHERE id=?`,
      args: [trade_id]
    });
    if(tradeForSignal.rows.length > 0) {
      const t = tradeForSignal.rows[0];
      if(t.signal_id) {
        // Signal exists — update it
        await db.execute({
          sql: `UPDATE signals SET outcome=?, exit_price=?, pnl_pct=?, evaluated_at=CURRENT_TIMESTAMP WHERE id=?`,
          args: [outcome, close_price||null, pnl_pct||null, t.signal_id]
        });
      }
      // No signal_id: trade outcome is already in active_trades table with pnl_pct
      // Don't insert duplicate - query will handle it separately
    }
...
app.post('/api/trades/close-by-symbol', async(req,res) => {
  ...
  try {
    ...
    const tradeForSignal2 = await db.execute({
      sql: `SELECT signal_id, direction, confidence FROM active_trades WHERE symbol=? AND status='CLOSED' ORDER BY id DESC LIMIT 1`,
      args: [symbol]
    });
    if(tradeForSignal2.rows.length > 0) {
      const t2 = tradeForSignal2.rows[0];
      if(t2.signal_id) {
        await db.execute({
          sql: `UPDATE signals SET outcome=?, exit_price=?, pnl_pct=?, evaluated_at=CURRENT_TIMESTAMP WHERE id=?`,
          args: [outcome2, close_price||null, pnl_pct||null, t2.signal_id]
        });
      }
      // No signal_id: already handled in active_trades table
    }
...
app.get('/api/signals/history', async(req,res) => {
  ...
  try {
    // Signals table: agent's learned trades (48 WIN + 23 LOSS = 71)
    const sigRows = await db.execute({
      sql: `SELECT symbol, signal as direction, confidence, outcome, pnl_pct,
    ...
    // Active trades: paper trading (NO overlap with signals, checked: overlap=0)
    // Include NULL signal_id trades (they don't have signals entries)
    const tradeRows = await db.execute({
      sql: `SELECT symbol, direction, confidence,
                   CASE WHEN pnl_pct >= 0 THEN 'WIN' ELSE 'LOSS' END as outcome,
                   pnl_pct, opened_at as created_at, closed_at as evaluated_at
            FROM active_trades
            WHERE status='CLOSED' AND pnl_pct IS NOT NULL
              AND (signal_id IS NULL OR signal_id NOT IN (SELECT id FROM signals WHERE outcome IN ('WIN','LOSS')))
            ORDER BY id DESC LIMIT 500`,
      args: []
    });
    
    // Merge: 71 + 88 = 159 (verified: no overlap)
    const all = [...sigRows.rows, ...tradeRows.rows];
    res.json({ok:true, signals: all});
  } catch(e) { res.status(500).json({ok:false,error:e.message}); }
});
...
// ── AUTO BACKTEST TOGGLE ─────────────────────────────────────────
let autoBacktestEnabled = true;

app.post('/api/backtest/auto-toggle', (req,res) => {
  const {enabled} = req.body;
  autoBacktestEnabled = !!enabled;
  res.json({ok:true, autoBacktestEnabled});
});

app.get('/api/backtest/auto-toggle', (req,res) => {
  res.json({ok:true, autoBacktestEnabled});
});
...
async function closeTrade(trade, exitPrice, reason) {
  // Note: Telegram sirf yahan se bhejte hain (server-side)
  // Frontend execExit ka telegram alag hai — dono mil ke double message nahi hona chahiye
  // Solution: closeTrade server DB se close karta hai, frontend bhi detect karta hai
  // Double message fix: server wala AUTHORITATIVE hai, frontend check karta hai db_trade_id
  ...
  if(pnlPct !== null) {
    ...
    if(trade.signal_id) {
      ...
      // FIX: SQLite UPDATE does NOT support LIMIT — use subquery instead
      await db.execute({
        sql: `UPDATE signals SET outcome=?, exit_price=?, pnl_pct=?, trade_type=?, evaluated_at=CURRENT_TIMESTAMP
              WHERE id = (
    ...
    // FIX: Update trade_journal exit — was missing, causing NULL outcomes
    // journal/exit only got called from frontend execExit, not from guardian auto-close
    try {
      await db.execute({
        // SQLite UPDATE doesn't support LIMIT directly — use subquery
        sql: `UPDATE trade_journal
              SET exit_price=?, exit_type=?, exit_reason=?,
                  pnl_pct=?, outcome=?, hold_time_minutes=?,
        ...
        args: [
          parseFloat(exitPrice.toFixed(8)),
          'AUTO',
          reason,
          pnlPct,
          pnlPct > 0 ? 'WIN' : 'LOSS',
          trade.opened_at ? Math.round((Date.now() - new Date(trade.opened_at)) / 60000) : 0,
          trade.symbol
        ]
      });
      console.log(`[JOURNAL] ${trade.symbol} exit logged: ${pnlPct > 0 ? 'WIN' : 'LOSS'} ${pnlPct}%`);
    } catch(journalErr) {
      console.warn('[JOURNAL] exit update error (non-critical):', journalErr.message);
    }
...
async function smartTradeGuardian() {
  if(!process.env.TURSO_DATABASE_URL) return;
  // marketCache empty ho to bhi chalo — price directly fetch karo
  if(!marketCache.length) {
    console.log('[GUARDIAN] marketCache empty — skipping this tick, will retry');
    return;
  }
  try {
    const rows = await db.execute({
    ...
    for(const trade of rows.rows){
      ...
      // ── 1. TRAILING STOP — FIXED % gap below/above price ──
      // Gain ke hisaab se fixed trail % decide karo (tight, consistent)
      const gainPct = isLong ? (price-entry)/entry*100 : (entry-price)/entry*100;
      if(gainPct >= trailAct){
        // Fixed trail percentage — NOT step-based
        // Tighter SL: only 0.8-2% away from current price
        let trailPct;
        if      (gainPct >= 8)  trailPct = 0.8;   // 0.8% below/above price
        else if (gainPct >= 6)  trailPct = 1.0;   // 1.0% below/above price
        else if (gainPct >= 4)  trailPct = 1.2;   // 1.2% below/above price
        else if (gainPct >= 2)  trailPct = 1.5;   // 1.5% below/above price
        else                    trailPct = 2.0;   // 2.0% below/above price (safety)

        // TP trail — conservative, har 0.5% gain = 0.5% TP extend (half of gain)
        const tpTrailPct = gainPct * 0.5; // slower than before (was gainStep * 1.0)
        ...
        let newSL, newTP;
        if(isLong){
          newSL = Math.max(sl, breakeven, price*(1-trailPct/100));
          ...
          if(slMoved || tpMoved){
            ...
            console.log('[GUARDIAN] TRAIL LONG '+trade.symbol+
              ' trail='+trailPct+'% SL→$'+sl.toFixed(6)+
              ' TP→$'+tp.toFixed(6)+
              ' (gain='+gainPct.toFixed(2)+'% gap=$'+(price-sl).toFixed(2)+')');
          ...
          newSL = Math.min(sl, breakeven, price*(1+trailPct/100));
          ...
          if(slMoved || tpMoved){
            ...
            console.log('[GUARDIAN] TRAIL SHORT '+trade.symbol+
              ' trail='+trailPct+'% SL→$'+sl.toFixed(6)+
              ' TP→$'+tp.toFixed(6)+
              ' (gain='+gainPct.toFixed(2)+'% gap=$'+(sl-price).toFixed(2)+')');
            // Live mode: update KuCoin stop order
            if(tradingMode==='live' && process.env.KUCOIN_API_KEY && trade.kucoin_sl_id && slMoved) {
              try {
                await cancelKuOrder(trade.kucoin_sl_id).catch(()=>{});
      ...
      // ── SMART EXIT ENGINE v2 (data-driven upgrade) ──────────────────
      // OLD: MOMENTUM_EXIT=0%WR(16 trades), NEAR_SL_CUT=0%WR(14 trades) → fixed
      // NEW: tighter conditions, profit protection, reversal confirmation required
      const m15e = coin.m15, h1e = coin.h1;
      if(m15e && h1e) {
        const lossPct  = gainPct < 0 ? Math.abs(gainPct) : 0;
        const inLoss   = gainPct < 0;
        const inProfit = gainPct > 0;

        // Indicator signals — reuse for both loss and profit exits
        const trendAgainst = isLong
          ? (h1e.trend==='DOWN' && m15e.trend==='DOWN')   // both TFs against
          : (h1e.trend==='UP'   && m15e.trend==='UP');
        const macdAgainst  = isLong ? (h1e.macdBull===false) : (h1e.macdBull===true);
        const momAgainst   = isLong ? (m15e.mom10 < -0.8) : (m15e.mom10 > 0.8); // tighter threshold
        const rsiAgainst   = isLong ? (h1e.rsi > 78)      : (h1e.rsi < 22);     // extreme only
        const belowVWAP    = isLong ? (price < (h1e.vwap||price)) : (price > (h1e.vwap||price));

        // Count strong reversal signals
        let reversalSignals = 0;
        if(trendAgainst) reversalSignals++;
        if(macdAgainst)  reversalSignals++;
        if(momAgainst)   reversalSignals++;
        if(rsiAgainst)   reversalSignals++;
        if(belowVWAP)    reversalSignals++;

        let earlyExitReason = null;

        // ── LOSS EXITS (only when trade is losing) ──
        if(inLoss) {
          // NEAR_SL_CUT v2: require MACD confirmation too (not just price proximity)
          if(lossPct >= slPct * 0.85 && macdAgainst && trendAgainst) {
            earlyExitReason = 'NEAR_SL_CUT';
          }
          // MOMENTUM_EXIT v2: need 3+ signals (was 1-2), much stricter
          else if(lossPct >= 1.2 && reversalSignals >= 3) {
            earlyExitReason = 'MOMENTUM_EXIT';
          }
          // TREND_REVERSAL: both timeframes flipped + MACD confirms
          else if(lossPct >= 0.8 && trendAgainst && macdAgainst && momAgainst) {
            earlyExitReason = 'TREND_REVERSAL';
          }
        }

        // ── PROFIT PROTECTION (ChatGPT Step 3 — secure winners) ──
        if(inProfit && !earlyExitReason) {
          // Reversal detected while in profit → lock gains
          if(reversalSignals >= 3 && gainPct > 0.5) {
            earlyExitReason = 'REVERSAL_PROFIT_LOCK';
          }
        }

        if(earlyExitReason) {
          console.log(`[GUARDIAN] SMART_EXIT ${trade.symbol} pnl=${gainPct.toFixed(2)}% reversals=${reversalSignals} reason=${earlyExitReason}`);
          await closeTrade(trade, price, earlyExitReason);
          continue;
        }
      }
...
// ── RESET: Close all open trades in DB (called by frontend resetAll) ──
app.post('/api/trades/reset-all', async(req,res) => {
  if (!process.env.TURSO_DATABASE_URL) return res.json({ok:true});
  try {
    const { cash } = req.body || {};
    const resetCash = (cash != null && !isNaN(parseFloat(cash))) ? parseFloat(cash) : 100;
    await db.execute({
      sql: `UPDATE active_trades SET status='CLOSED', close_reason='RESET', closed_at=CURRENT_TIMESTAMP WHERE status='OPEN'`,
      args: []
    });
    // ✅ Cash bhi atomically reset karo — agar portfolio/save fail bhi ho toh DB correct rahega
    await db.execute({
      sql: `INSERT INTO portfolio_state (id, cash, updated_at) VALUES (1, ?, CURRENT_TIMESTAMP)
            ON CONFLICT(id) DO UPDATE SET cash=excluded.cash, updated_at=CURRENT_TIMESTAMP`,
      args: [resetCash]
    });
    console.log('[RESET] All open trades closed in DB, cash reset to', resetCash);
    res.json({ok:true});
  } catch(e) {
    console.warn('[RESET] Close all error:', e.message);
    res.json({ok:false, error: e.message});
  }
});

// ════════════════════════════════════════════════════════════════
// PHASE 2: TRADE JOURNAL & ANALYTICS ENDPOINTS
// ════════════════════════════════════════════════════════════════

app.post('/api/trades/journal/entry', async(req,res) => {
  if (!process.env.TURSO_DATABASE_URL) return res.json({ok:false,error:'DB not configured'});
  try {
    const {symbol, entry_price, entry_confidence, entry_reason, regime, exchange} = req.body;
    if (!symbol || !entry_price) return res.status(400).json({ok:false,error:'Missing required fields'});
    const result = await db.execute({
      sql: `INSERT INTO trade_journal
            (symbol, entry_price, entry_confidence, entry_reason, regime, mode, exchange)
            VALUES (?, ?, ?, ?, ?, ?, ?)`,
      args: [symbol, entry_price, entry_confidence||0, entry_reason||'AUTO', regime||'SIDE',
             tradingMode, exchange||'kucoin']
    });
    res.json({ok:true, journal_id: Number(result.lastInsertRowid)});
  } catch(e) {
...
app.post('/api/trades/journal/exit', async(req,res) => {
  if (!process.env.TURSO_DATABASE_URL) return res.json({ok:false,error:'DB not configured'});
  try {
    const {symbol, exit_price, exit_type, exit_reason, pnl_pct} = req.body;
    if (!symbol || !exit_price) return res.status(400).json({ok:false,error:'Missing fields'});
    const trade = await db.execute({
      sql: `SELECT id, entry_time, entry_price FROM trade_journal
            WHERE symbol=? AND exit_time IS NULL
            ORDER BY id DESC LIMIT 1`,
      args: [symbol]
    });
    if (!trade.rows.length) return res.status(400).json({ok:false,error:'No open trade found'});
    const t = trade.rows[0];
    const entryTime = new Date(t.entry_time);
    const holdMinutes = Math.round((Date.now() - entryTime) / 60000);
    const realPnL = pnl_pct != null ? parseFloat(pnl_pct) :
      ((exit_price - t.entry_price) / t.entry_price * 100);
    const outcome = realPnL >= 0 ? 'WIN' : 'LOSS';
    await db.execute({
      sql: `UPDATE trade_journal
            SET exit_price=?, exit_type=?, exit_reason=?,
                pnl_pct=?, outcome=?, hold_time_minutes=?,
                exit_time=CURRENT_TIMESTAMP, updated_at=CURRENT_TIMESTAMP
            WHERE id=?`,
      args: [exit_price, exit_type||'MANUAL', exit_reason||'', realPnL, outcome, holdMinutes, t.id]
    });
    res.json({ok:true, outcome, pnl_pct: realPnL, hold_minutes: holdMinutes});
  } catch(e) {
    console.error('journal/exit error:', e.message);
    res.status(500).json({ok:false,error:e.message});
  }
});

app.get('/api/analytics/wr-by-regime', async(req,res) => {
  if (!process.env.TURSO_DATABASE_URL) return res.json({ok:true, data:{}});
  try {
    const result = await db.execute({
      sql: `SELECT regime,
                   COUNT(*) as total,
                   COUNT(CASE WHEN outcome='WIN' THEN 1 END) as wins,
                   ROUND(COUNT(CASE WHEN outcome='WIN' THEN 1 END)*100.0/COUNT(*), 1) as wr_pct
            FROM trade_journal
            WHERE outcome IS NOT NULL AND regime IS NOT NULL
            GROUP BY regime ORDER BY wr_pct DESC`
    });
    const data = {};
    result.rows.forEach(row => { data[row.regime] = {total:row.total, wins:row.wins, wr:row.wr_pct}; });
    res.json({ok:true, data});
  } catch(e) { res.json({ok:true, data:{}}); }
});

app.get('/api/analytics/wr-by-confidence', async(req,res) => {
  if (!process.env.TURSO_DATABASE_URL) return res.json({ok:true, data:{}});
  try {
    const result = await db.execute({
      sql: `SELECT
                   CASE
                     WHEN entry_confidence >= 90 THEN '90+'
                     WHEN entry_confidence >= 80 THEN '80-89'
                     WHEN entry_confidence >= 70 THEN '70-79'
                     ELSE '<70'
                   END as conf_bracket,
                   COUNT(*) as total,
                   COUNT(CASE WHEN outcome='WIN' THEN 1 END) as wins,
    ...
    const data = {};
    result.rows.forEach(row => { data[row.conf_bracket] = {total:row.total, wins:row.wins, wr:row.wr_pct}; });
    res.json({ok:true, data});
  } catch(e) { res.json({ok:true, data:{}}); }
});

app.get('/api/analytics/wr-by-coin', async(req,res) => {
  if (!process.env.TURSO_DATABASE_URL) return res.json({ok:true, data:{}});
  try {
    const result = await db.execute({
      sql: `SELECT symbol,
            ...
            ORDER BY wr_pct DESC LIMIT 20`
    });
    const data = {};
    result.rows.forEach(row => { data[row.symbol] = {total:row.total, wins:row.wins, wr:row.wr_pct}; });
    res.json({ok:true, data});
  } catch(e) { res.json({ok:true, data:{}}); }
});

app.get('/api/analytics/wr-by-reason', async(req,res) => {
  if (!process.env.TURSO_DATABASE_URL) return res.json({ok:true, data:{}});
  try {
    const result = await db.execute({
      sql: `SELECT entry_reason,
                   COUNT(*) as total,
                   COUNT(CASE WHEN outcome='WIN' THEN 1 END) as wins,
                   ROUND(COUNT(CASE WHEN outcome='WIN' THEN 1 END)*100.0/COUNT(*), 1) as wr_pct
            FROM trade_journal WHERE outcome IS NOT NULL AND entry_reason IS NOT NULL
            GROUP BY entry_reason ORDER BY wr_pct DESC`
    });
    const data = {};
    result.rows.forEach(row => { data[row.entry_reason] = {total:row.total, wins:row.wins, wr:row.wr_pct}; });
    res.json({ok:true, data});
  } catch(e) { res.json({ok:true, data:{}}); }
});
...
app.get('/api/analytics/trades', async(req,res) => {
  ...
  try {
    ...
    const result = await db.execute({sql:`SELECT * FROM trade_journal ORDER BY id DESC LIMIT ?`, args:[limit]});
    res.json({ok:true, trades: result.rows});
  } catch(e) { res.json({ok:true, trades:[]}); }
});

app.get('/api/analytics/summary', async(req,res) => {
  if (!process.env.TURSO_DATABASE_URL) return res.json({ok:true, summary:{}});
  try {
    const result = await db.execute({
      sql: `SELECT
                   ...
                   ROUND(COUNT(CASE WHEN outcome='WIN' THEN 1 END)*100.0/NULLIF(COUNT(*),0), 1) as overall_wr,
                   ROUND(AVG(pnl_pct), 2) as avg_pnl,
    ...
    res.json({ok:true, summary:{
      ...
      win_loss_ratio: row.total_losses > 0 && row.avg_loss_pct ?
        (row.avg_win_pct / Math.abs(row.avg_loss_pct)).toFixed(2) : 'N/A'
    }});
  } catch(e) { res.json({ok:true, summary:{}}); }
});

app.post('/api/smart-rules/check-entry', async(req,res) => {
  if (!process.env.TURSO_DATABASE_URL) return res.json({ok:true, allowed:true, reason:'DB disabled'});
  try {
    const {symbol, confidence, regime} = req.body;
    const reasons = [];
    let allowed = true;

    // Rule 1: Min confidence 6.5
    if (confidence < 6.5) { allowed=false; reasons.push(`Low confidence (${confidence}<6.5)`); }

    // Rule 2: Check coin historical WR from signals table (real data, min 3 trades)
    const coinData = await db.execute({
      sql: `SELECT COUNT(*) as total,
                   SUM(CASE WHEN outcome='WIN' THEN 1 ELSE 0 END) as wins
            FROM signals WHERE symbol=? AND outcome IN ('WIN','LOSS')`,
      args: [symbol]
    });
    if (coinData.rows.length && coinData.rows[0].total >= 1) {
      const wr = (coinData.rows[0].wins / coinData.rows[0].total * 100);
      // 0 wins on ANY trades = block immediately
      if (coinData.rows[0].wins === 0) {
        allowed=false; reasons.push(`Zero wins on ${symbol} (${coinData.rows[0].total} trades) — blocked`);
      } else if (coinData.rows[0].total >= 3 && wr < 35) {
        allowed=false; reasons.push(`Low coin WR ${wr.toFixed(0)}% (<35%) on ${coinData.rows[0].total} trades`);
      }
    }

    // Rule 3: Skip coins on consecutive losses (last 3 trades all LOSS)
    const recentTrades = await db.execute({
      sql: `SELECT outcome FROM signals WHERE symbol=? AND outcome IN ('WIN','LOSS') ORDER BY id DESC LIMIT 3`,
      args: [symbol]
    });
    if (recentTrades.rows.length >= 3) {
      const allLoss = recentTrades.rows.every(r => r.outcome === 'LOSS');
      if (allLoss) { allowed=false; reasons.push(`3 consecutive losses on ${symbol} — cooling down`); }
    }

    res.json({ok:true, allowed, reasons, recommendation: allowed?'ENTER':'SKIP'});
  } catch(e) { res.json({ok:true, allowed:true, reason:'Error checking rules'}); }
});
...
// ════════════════════════════════════════════════════════════════
// PHASE 3: UI/MONITORING — Trade Alerts + CSV Export
// ════════════════════════════════════════════════════════════════

// Trade Entry Alert — send to Telegram when new trade opens
app.post('/api/trades/alert-entry', async(req,res)=>{
  if(!TG_TOKEN || !TG_CHAT) return res.json({ok:true});
  try {
    const {symbol, direction, entry_price, tp_price, sl_price, confidence, reasoning} = req.body;
    const dir = direction === 'LONG' ? '🟢 LONG' : '🔴 SHORT';
    const tpRR = tp_price && sl_price ? ((tp_price - entry_price) / Math.abs(entry_price - sl_price)).toFixed(2) : 'N/A';
    const msg = `${dir} <b>${symbol}</b> @ $${parseFloat(entry_price).toFixed(4)}
TP: $${parseFloat(tp_price).toFixed(4)} | SL: $${parseFloat(sl_price).toFixed(4)} | RR: ${tpRR}:1
Confidence: ${confidence}/10 | Reason: ${reasoning || 'AI Signal'}`;
    await sendTelegram(msg);
    res.json({ok:true});
  } catch(e) {
    console.warn('[TRADE ALERT ENTRY]', e.message);
    res.json({ok:false, error:e.message});
  }
});

// Trade Exit Alert — DISABLED: double message fix
// closeTrade() server-side already sends Telegram on exit.
// Frontend calling this caused 2 messages per trade (EXIT + TRADE CLOSED).
app.post('/api/trades/alert-exit', async(req,res)=>{
  // Silently skip — server closeTrade() is the authoritative Telegram source for exits
  res.json({ok:true, skipped:'closeTrade handles exit notification'});
});

// CSV Export — download all trades as CSV
// ── AUTO OPTIMIZER STATUS ─────────────────────────────────────────────
app.get('/api/optimizer/status', (req,res) => {
  res.json({ ok: true, params: autoParams });
});
...
app.get('/api/export/csv', async(req,res)=>{
  if(!process.env.TURSO_DATABASE_URL) return res.json({ok:false, error:'DB not available'});
  try {
    const rows = await db.execute({
      sql: `SELECT 
        symbol, direction, entry_price, exit_price, sl_price, tp_price,
        confidence, pnl_pct, close_reason, reasoning,
        opened_at, closed_at, status, trade_type, leverage, exchange
      FROM active_trades
    ...
    // Build CSV header
    const headers = ['Symbol', 'Direction', 'Entry Price', 'Exit Price', 'SL', 'TP', 
                     'Confidence', 'P&L %', 'Status', 'Opened', 'Closed', 'Close Reason', 'Reasoning',
                     'Type', 'Leverage', 'Exchange'];
    ...
    trades.forEach(t => {
      const row = [
        ...
        parseFloat(t.exit_price || 0).toFixed(4) || '',
        parseFloat(t.sl_price || 0).toFixed(4),
        parseFloat(t.tp_price || 0).toFixed(4),
        t.confidence || '',
    ...
    res.setHeader('Content-Disposition', `attachment; filename="nexus-trades-${new Date().toISOString().split('T')[0]}.csv"`);
    res.send(csv.join('\n'));
  } catch(e) {
    console.warn('[CSV EXPORT]', e.message);
    res.json({ok:false, error:e.message});
  }
});

// Enhanced Daily Report with equity metrics
app.get('/api/reports/daily', async(req,res)=>{
  if(!process.env.TURSO_DATABASE_URL) return res.json({ok:false});
  try {
    const rows = await db.execute({
      sql: `SELECT 
        COUNT(*) as total,
...
// 🔥 UPGRADE: Backtesting Analysis Endpoint - Deep analysis of historical performance
app.get('/api/backtest/analyze', async(req,res)=>{
  if(!process.env.TURSO_DATABASE_URL) return res.json({ok:false});
  try {
    // Get all closed trades
    const rows = await db.execute({
      sql: `SELECT symbol, direction, entry_price, exit_price, pnl_pct, opened_at, closed_at,
            confidence, reasoning, trade_type
            FROM active_trades 
            WHERE status = 'CLOSED'
    ...
    const trades = rows.rows || [];
    if(trades.length < 5) return res.json({ok:false, message:'Need 5+ closed trades'});
    ...
    const avgPnl = (trades.reduce((s,t)=>s+(t.pnl_pct||0), 0) / trades.length).toFixed(2);
    const bestTrade = Math.max(...trades.map(t=>t.pnl_pct||0)).toFixed(2);
    const worstTrade = Math.min(...trades.map(t=>t.pnl_pct||0)).toFixed(2);
    const profitFactor = losses === 0 ? '∞' : 
      (trades.filter(t=>t.pnl_pct>0).reduce((s,t)=>s+t.pnl_pct,0) / 
       Math.abs(trades.filter(t=>t.pnl_pct<0).reduce((s,t)=>s+t.pnl_pct,0))).toFixed(2);
    
    // By regime analysis would need regime field - for now by direction
    ...
    const bySymbol = {};
    trades.forEach(t => {
      if(!bySymbol[t.symbol]) bySymbol[t.symbol] = {wins:0, losses:0, pnl:0, trades:[]};
      bySymbol[t.symbol].trades.push(t);
      if(t.pnl_pct >= 0) bySymbol[t.symbol].wins++;
      else bySymbol[t.symbol].losses++;
      bySymbol[t.symbol].pnl += t.pnl_pct;
    });
    
    // Top performers
    const topCoins = Object.entries(bySymbol)
      .map(([sym, stats]) => ({
        symbol: sym,
    ...
    if(wr > 50) recommendations.push(`✅ Win rate ${wr}% is good - keep entry rules strict`);
    if(wr < 40) recommendations.push(`⚠️ Win rate ${wr}% below 40% - need stricter confluence`);
    if(profitFactor > 2) recommendations.push(`✅ Profit factor ${profitFactor}x excellent`);
    if(profitFactor < 1.5 && profitFactor !== '∞') recommendations.push(`⚠️ Profit factor low - need better exits`);
    if(longTrades.length > shortTrades.length * 2) recommendations.push(`ℹ️ Heavy bias to LONGs - consider market regime`);
    ...
    res.json({
      ...
      byDirection: {
        long: {count: longTrades.length, wr: longTrades.length > 0 ? 
          ((longTrades.filter(t=>t.pnl_pct>=0).length/longTrades.length)*100).toFixed(0)+'%' : 'N/A'},
        short: {count: shortTrades.length, wr: shortTrades.length > 0 ? 
          ((shortTrades.filter(t=>t.pnl_pct>=0).length/shortTrades.length)*100).toFixed(0)+'%' : 'N/A'}
      },
      topCoins,
      recommendations
    });
  } catch(e) {
    console.warn('[BACKTEST]', e.message);
    res.json({ok:false, error:e.message});
  }
});

// ── FORECAST: TECHNICAL ANALYSIS (FREE - No Claude) ──────────────
app.post('/api/forecast/technical', async (req, res) => {
  try {
    const { symbol } = req.body;
    if (!symbol) return res.status(400).json({ ok: false, error: 'Symbol required' });

    const coin = marketCache.find(c => c.symbol === symbol.toUpperCase());
    if (!coin) return res.status(404).json({ ok: false, error: 'Coin not found in market cache' });

    const pf = preFilter(coin, true, true);
    const vf = passVolatilityFilter(coin);
    ...
    if (h4.trend === 'UP') upScore += 2;
    if (h1.trend === 'UP') upScore += 2;
    if (m15.trend === 'UP') upScore += 1;
    if (h4.trend === 'DOWN') downScore += 2;
    if (h1.trend === 'DOWN') downScore += 2;
    if (m15.trend === 'DOWN') downScore += 1;
    if (h1.macdBull) upScore += 1; else downScore += 1;
    if (h1.rsi > 50) upScore += 1; else downScore += 1;
    if (d1.trend === 'UP') upScore += 1;
    if (d1.trend === 'DOWN') downScore += 1;
    const price = coin.price || 0;
    if (h1.vwap > 0 && price > h1.vwap) upScore += 1;
    if (h1.vwap > 0 && price < h1.vwap) downScore += 1;

    const totalScore = upScore + downScore || 1;
    const direction = upScore >= downScore ? 'UP' : 'DOWN';
    const confidence = Math.round((Math.max(upScore, downScore) / totalScore) * 100);

    // History from DB
    const histLong  = await getSimilarTrades(symbol.toUpperCase(), 'LONG', 10);
    const histShort = await getSimilarTrades(symbol.toUpperCase(), 'SHORT', 10);
    const histLongWR  = histLong.length ? Math.round(histLong.filter(t => t.outcome === 'WIN').length / histLong.length * 100) : null;
    const histShortWR = histShort.length ? Math.round(histShort.filter(t => t.outcome === 'WIN').length / histShort.length * 100) : null;

    const adaptiveTP = direction === 'UP'
      ? await getAdaptiveTPSL(symbol.toUpperCase(), 'LONG')
      : await getAdaptiveTPSL(symbol.toUpperCase(), 'SHORT');
    ...
    res.json({
      ...
      indicators: {
        ...
        rsi:     h1.rsi || 'N/A',
        macd:    h1.macdBull ? 'BULL' : 'BEAR',
        bbPos:   h1.bbPos || 'N/A',
        stochK:  h1.stochK || 'N/A',
        vwap:    h1.vwap || 'N/A',
        volRatio: m15.volRatio || 'N/A',
        pattern:  m15.pattern || 'NONE',
        williamsR: h1.williamsR || 'N/A',
      },
      history: {
        longTrades: histLong.length, longWR: histLongWR,
        shortTrades: histShort.length, shortWR: histShortWR,
      },
      adaptiveTP: adaptiveTP.tp || null,
      adaptiveSL: adaptiveTP.sl || null,
    });
  } catch (e) {
    console.error('[FORECAST] Technical error:', e.message);
    res.status(500).json({ ok: false, error: e.message });
  }
});

// ── FORECAST: CLAUDE DEEP ANALYSIS (ON-DEMAND, $$ cost) ──────────
app.post('/api/forecast/claude', async (req, res) => {
  if (!process.env.ANTHROPIC_API_KEY) return res.status(500).json({ ok: false, error: 'CLAUDE_API_KEY not set' });
  try {
    const { symbol } = req.body;
    if (!symbol) return res.status(400).json({ ok: false, error: 'Symbol required' });
    ...
    const prompt = `Analyze ${symbol} for trading forecast:
Price: $${coin.price}
H1 Trend: ${h1.trend}, RSI: ${h1.rsi}, MACD: ${h1.macdBull ? 'BULL' : 'BEAR'}
H4 Trend: ${h4.trend}
Prefilter Score: ${pf.score}/100
Reason: ${pf.reasons}

Give SHORT (3-4 line) analysis: entry direction, risk level, key level, probability.`;
    ...
    res.json({ ok: true, symbol: symbol.toUpperCase(), analysis, cost: '$0.002' });
  } catch (e) {
    console.error('[FORECAST] Claude error:', e.message);
    res.status(500).json({ ok: false, error: e.message });
  }
});
...
app.listen(PORT,async()=>{
  ...
  // ── AUTO STRATEGY OPTIMIZER — runs every 30 min ──────────────────
  setTimeout(async () => {
    await autoOptimize();
    setInterval(autoOptimize, 30 * 60 * 1000);
  }, 5 * 60 * 1000); // first run after 5 min (let data load)

  // ── SERVER-SIDE AUTO TRADING CYCLE ─────────────────────────────
  // Runs every 5 min — browser doesn't need to be open!
  setInterval(async () => {
    try {
      if (tradingMode !== 'live') return;
      if (!marketCache.length) return;
      if (!CLAUDE_KEY) return;

      // Max 3 open trades
      const openRes = await db.execute({sql:`SELECT COUNT(*) as cnt FROM active_trades WHERE status='OPEN'`,args:[]});
      const openCount = openRes.rows[0]?.cnt || 0;
      if (openCount >= 3) { console.log(`[AUTO CYCLE] Skip - ${openCount}/3 trades open`); return; }

      // Drawdown check
      const ddHit = await checkDrawdown(5);
      if (ddHit) { console.log(`[AUTO CYCLE] Skip - DD limit`); return; }

      // Regime - same as frontend (top 15 coins h4+h1)
      let abull=0, abear=0, atotal=0;
      marketCache.slice(0,15).forEach(c=>{
        if(!c) return;
        const t4=c.h4?.trend, t1=c.h1?.trend, tm=c.m15?.trend;
        atotal++;
        if(t4==='UP'&&(t1==='UP'||tm==='UP')) abull++;
        else if(t4==='DOWN'&&(t1==='DOWN'||tm==='DOWN')) abear++;
      });
      const regime = atotal>0
        ? (abull/atotal>=0.25?'BULL':abear/atotal>=0.25?'BEAR':'SIDE')
        : 'SIDE';

      // Prefilter top 5 candidates
      let candidates = marketCache.slice(0,30).map(coin=>{
        const vf = passVolatilityFilter(coin);
        if(!vf.pass) return null;
        const pf = preFilter(coin, autoParams.aggressiveMode, false);
        if(!pf.score||pf.score<autoParams.scoreThreshold) return null;
      ...
      // Regime filter v2 — flexible (ChatGPT suggestion implemented)
      // OLD: BULL = hard block SHORT, BEAR = hard block LONG
      // NEW: opposing trades allowed at LOW_CONFIDENCE (half size) if strong setup
      candidates = candidates.filter(c=>{
        if(regime==='BEAR' && c.direction==='LONG') {
          if(c.score >= 90) {
            // Very strong LONG setup in BEAR — allow at reduced confidence
            c.lowConfidenceOverride = true;
            c.overrideReason = 'BEAR regime — size halved';
            console.log(`[AUTO] BEAR override: ${c.symbol} LONG score=${c.score} (high score, half size)`);
            return true;
          }
          console.log(`[AUTO] SKIP ${c.symbol} LONG - BEAR (score=${c.score}<90)`);
          return false;
        }
        if(regime==='BULL' && c.direction==='SHORT') {
          if(c.score >= 90) {
            // Very strong SHORT setup in BULL — allow at reduced confidence
            c.lowConfidenceOverride = true;
            c.overrideReason = 'BULL regime — size halved';
            console.log(`[AUTO] BULL override: ${c.symbol} SHORT score=${c.score} (high score, half size)`);
            return true;
          }
          console.log(`[AUTO] SKIP ${c.symbol} SHORT - BULL (score=${c.score}<90)`);
          return false;
        }
        return true;
      });

      if(!candidates.length){ console.log(`[AUTO CYCLE] No candidates | Regime=${regime}`); return; }
      console.log(`[AUTO CYCLE] ${candidates.length} candidates | Regime=${regime} | Open=${openCount}/3`);

      // ── FIX 2: Learning data inject — per candidate history ──────────
      const recentStats = await getRecentStats();
      const learningLines = await Promise.all(candidates.map(async c => {
        try {
          const hist = await getSimilarTrades(c.symbol, c.direction, 5);
          const wins = hist.filter(t=>t.outcome==='WIN').length;
          const total = hist.length;
          const wr = total>0 ? `${((wins/total)*100).toFixed(0)}%WR(${total}trades)` : 'no_history';
          return `${c.symbol} ${c.direction} score=${c.score} price=${c.price} history=${wr}: ${c.reasons}`;
        } catch(e){ return `${c.symbol} ${c.direction} score=${c.score} price=${c.price} history=no_history: ${c.reasons}`; }
      }));

      // Claude evaluation — enriched prompt with learning context
      const prompt = `Regime:${regime}. GlobalWR:${recentStats.winRate}%(${recentStats.total}trades).\nAnalyze:\n${learningLines.join('\n')}\nPick best 1-2 ONLY if confidence>=7.0. Prefer coins with positive history. Skip weak setups. ${regime==='BEAR'?'Prefer SHORT. LONG only if score>=90 (half size).':regime==='BULL'?'Prefer LONG. SHORT only if score>=90 (half size).':''}`;
      const r = await axios.post('https://api.anthropic.com/v1/messages',{
        model:'claude-opus-4-5',max_tokens:400,
        system:`Trading AI. Return ONLY JSON: {"signals":[{"symbol":"","direction":"","confidence":7.0,"sl_pct":2.0,"tp_pct":5.0,"reason":"","trade_type":"futures","leverage":5}],"skip_reason":""}. Only pick HIGH confidence trades. When in doubt, use skip_reason.`,
        messages:[{role:'user',content:prompt}]
      },{headers:{'x-api-key':CLAUDE_KEY,'anthropic-version':'2023-06-01','Content-Type':'application/json'},timeout:25000});

      const m = r.data.content[0].text.match(/\{[\s\S]*\}/);
      if(!m) return;
      const parsed = JSON.parse(m[0]);
      if(!parsed.signals?.length){ console.log(`[AUTO CYCLE] Claude skip: ${parsed.skip_reason||'-'}`); return; }

      for(const sig of parsed.signals){
        if(!['LONG','SHORT'].includes(sig.direction)||!sig.symbol) continue;
        // ── FIX 1: Confidence threshold raised 6.0 → 7.0 ────────────────
        if((sig.confidence||0)<autoParams.confidenceThreshold) { console.log(`[AUTO CYCLE] Skip ${sig.symbol} — conf ${sig.confidence} < ${autoParams.confidenceThreshold} (autoParam)`); continue; }
        const hasStreak = await checkLossStreak(sig.symbol);
        if(hasStreak) continue;

        const coin = marketCache.find(c=>c.symbol===sig.symbol);
        if(!coin?.price) continue;

        // ── FIX 4: Adaptive TP/SL from historical wins ───────────────────
        let sl_pct = sig.sl_pct||2.0, tp_pct = sig.tp_pct||5.0;
        try {
          const { tp: adaptTP, sl: adaptSL } = await getAdaptiveTPSL(sig.symbol, sig.direction);
          sl_pct = adaptSL;
          tp_pct = adaptTP;
          console.log(`[AUTO ADAPT] ${sig.symbol} ${sig.direction}: TP=${tp_pct.toFixed(2)}% SL=${sl_pct.toFixed(2)}%`);
        } catch(e){ console.log(`[AUTO ADAPT] ${sig.symbol} — using defaults`); }

        const price=coin.price;
        const slPrice=parseFloat((price*(sig.direction==='LONG'?1-sl_pct/100:1+sl_pct/100)).toFixed(6));
        const tpPrice=parseFloat((price*(sig.direction==='LONG'?1+tp_pct/100:1-tp_pct/100)).toFixed(6));
        ...
        let exchange='kucoin';
        if(kuOk&&bxOk){
          const hash=sig.symbol.split('').reduce((a,b)=>a+b.charCodeAt(0),0);
          global.exchCounter=(global.exchCounter||0)+1; exchange=global.bxOnlyCoins?.has(sig.symbol)?'bingx':(global.exchCounter%2===0?'bingx':'kucoin');
        } else if(bxOk) exchange='bingx';

        // ── REGIME OVERRIDE: Half size for opposing direction in strong regime ──
        if(sig.lowConfidenceOverride) {
          sig.size = sig.size ? sig.size * 0.5 : null;
          sig.confidence = Math.min(sig.confidence || 7, 6.5); // cap confidence
          console.log(`[REGIME OVERRIDE] ${sig.symbol} ${sig.direction} — half size, conf capped 6.5`);
        }
        ...
        try {
          const dupRow = await db.execute({
            sql:`SELECT id FROM active_trades WHERE status='OPEN' AND symbol=? AND exchange=? LIMIT 1`,
            args:[sig.symbol, exchange]
          });
          if(dupRow.rows.length > 0){
            console.log(`[DUP BLOCK] ${sig.symbol} already OPEN on ${exchange.toUpperCase()} — skipped`);
            continue;
          }
        } catch(e){ console.warn('[DUP BLOCK] check failed:', e.message); }

        // Size
        const port = await db.execute({sql:`SELECT cash FROM portfolio_state ORDER BY id DESC LIMIT 1`,args:[]});
        const equity=port.rows[0]?.cash||100;
        const size=Math.max(5,Math.min(equity*0.1,20));

        console.log(`[AUTO TRADE] ${sig.symbol} ${sig.direction} conf=${sig.confidence} → ${exchange.toUpperCase()} $${size}`);

        await axios.post(`http://localhost:${PORT}/api/trades/open`,{
          symbol:sig.symbol, direction:sig.direction, entry_price:price,
          tp_price:tpPrice, sl_price:slPrice, sl_pct, tp_pct,
          confidence:sig.confidence, size_usdt:size,
          reasoning:sig.reason||`Auto:${sig.direction} ${regime}`,
          trade_type:sig.trade_type||'futures', leverage:sig.leverage||5, exchange
        });
      }
    } catch(e){ console.error('[AUTO CYCLE ERROR]',e.message); }
  }, 5*60*1000);
