...
app.listen(PORT,async()=>{
  ...
  setInterval(async () => {
    try {
      ...
      // Claude evaluation
      const prompt = `Regime:${regime}. Analyze:\n${candidates.map(c=>`${c.symbol} ${c.direction} score=${c.score} price=${c.price}: ${c.reasons}`).join('\n')}\nPick best 1-2 with confidence>=6.5. ${regime==='BEAR'?'SHORT only.':regime==='BULL'?'LONG only.':''}`;
      const r = await axios.post('https://api.anthropic.com/v1/messages',{
        model:'claude-opus-4-5',max_tokens:400,
        system:`Trading AI. Return ONLY JSON: {"signals":[{"symbol":"","direction":"","confidence":7.0,"sl_pct":2.0,"tp_pct":5.0,"reason":"","trade_type":"futures","leverage":5}],"skip_reason":""}`,
        messages:[{role:'user',content:prompt}]
      ...
      for(const sig of parsed.signals){
        ...
        if((sig.confidence||0)<6.0) continue;
        ...
        const price=coin.price, sl_pct=sig.sl_pct||2.0, tp_pct=sig.tp_pct||5.0;
