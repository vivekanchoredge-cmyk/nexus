...
async function initialLoad(){
  ...
  try {
    ...
    bxCoins = tickers
      .filter(t=>t.symbol&&t.symbol.endsWith('-USDT')&&!exclude.some(e=>t.symbol.replace('-USDT','').includes(e))&&parseFloat(t.quoteVolume||0)>5000000)
      .sort((a,b)=>parseFloat(b.quoteVolume||0)-parseFloat(a.quoteVolume||0))
      .slice(0,30).map(t=>t.symbol.replace('-USDT',''));
...
app.post('/api/order',async(req,res)=>{
  ...
  try {
    ...
    const kuOk=!!(process.env.KUCOIN_API_KEY&&process.env.KUCOIN_API_SECRET&&process.env.KUCOIN_PASSPHRASE);
    const bxOk=!!(BINGX_API_KEY&&BINGX_SECRET);
    // Use BingX if KuCoin keys missing OR coin only on BingX
    ...
    if(useBingX){
      ...
      entryId=r?.orderId||r?.clientOrderID||'bx_'+Date.now();
      // BingX spot doesn't support stop orders via same API — SL/TP managed by Guardian
    } else {
      exchange='kucoin';
    ...
    res.json({ok:true,entryId,slId,tpId,exchange});
  } catch(e){res.status(500).json({ok:false,error:e.message});}
});
