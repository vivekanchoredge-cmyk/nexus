...
async function bxRequest(method, path, params={}) {
  ...
  if(r.data.code !== 0) throw new Error(`BingX error: ${r.data.msg}`);
  return r.data.data;
}
...
async function fetchBingXTickers() {
  const r = await axios.get('https://open-api.bingx.com/openApi/spot/v2/ticker/24hr', { timeout:10000 });
  if(r.data.code !== 0) throw new Error('BingX tickers failed');
  return r.data.data || [];
}
...
async function bxFuturesRequest(method, path, params={}) {
  ...
  const url = `https://open-api.bingx.com${path}?${qs}&signature=${sig}`;
  const r = await axios({ method, url, headers:{'X-BX-APIKEY': BINGX_API_KEY}, timeout:10000 });
  if(r.data.code !== 0) throw new Error(`BingX futures error: ${r.data.msg}`);
  return r.data.data;
}
...
async function initialLoad(){
  ...
  console.log(`Total coins: ${activeSymbols.length} (KU:${kuCoins.length} BX:${bxCoins.length})`);
  let loaded=0;
