    ...
    if(process.env.TURSO_DATABASE_URL&&Array.isArray(parsed.signals)&&parsed.signals.length>0){
      ...
      for(const sig of parsed.signals){
        ...
        try {
          ...
          const candles = coin?.m15?.candles || [];
...
app.post('/api/forecast/claude', async (req, res) => {
  ...
  try {
    ...
    const pf = preFilter(coin, true, true);
    const h1 = coin.h1 || {}, h4 = coin.h4 || {}, m15 = coin.m15 || {}, d1 = coin.d1 || {};

    const histLong  = await getSimilarTrades(symbol.toUpperCase(), 'LONG', 5);
    const histShort = await getSimilarTrades(symbol.toUpperCase(), 'SHORT', 5);
    const recentStats = await getRecentStats();
    const histLongWR  = histLong.length ? Math.round(histLong.filter(t => t.outcome === 'WIN').length / histLong.length * 100) : 'N/A';
    const histShortWR = histShort.length ? Math.round(histShort.filter(t => t.outcome === 'WIN').length / histShort.length * 100) : 'N/A';

    const prompt = `Analyze ${symbol.toUpperCase()} for a trading forecast:

PRICE: $${coin.price}
INDICATORS:
- Trends: m15=${m15.trend} h1=${h1.trend} h4=${h4.trend} d1=${d1.trend}
- RSI(h1): ${h1.rsi}
- MACD: ${h1.macdBull ? 'BULLISH' : 'BEARISH'} | CrossUp=${h1.macdCrossUp} CrossDn=${h1.macdCrossDn}
- BB Position: ${h1.bbPos}%
- Stoch K: ${h1.stochK} | CrossUp=${h1.stochCrossUp} CrossDn=${h1.stochCrossDn}
- VWAP: ${h1.vwap} (price ${coin.price > h1.vwap ? 'ABOVE' : 'BELOW'})
- Volume Ratio: ${m15.volRatio}x
- Pattern: ${m15.pattern || 'NONE'}
- Williams%R: ${h1.williamsR}
- Prefilter Score: ${pf.score || 0} | Direction: ${pf.direction || 'N/A'}

LEARNING DATA:
- Total trades: ${recentStats.total} (${recentStats.winRate}% WR)
- ${symbol} LONG history: ${histLong.length} trades, WR=${histLongWR}%
- ${symbol} SHORT history: ${histShort.length} trades, WR=${histShortWR}%

Provide a concise forecast with:
1. Direction (UP/DOWN/NEUTRAL) with confidence %
2. Key reasons (2-3 bullet points)
3. Risk level (LOW/MEDIUM/HIGH)
4. Suggested action (BUY/SELL/WAIT)
...
Keep response concise and actionable.`;
    ...
    const analysis = r.data?.content?.[0]?.text || 'No analysis available';
    res.json({ ok: true, symbol: symbol.toUpperCase(), analysis, price: coin.price });
  } catch (e) {
...
app.post('/api/smart-rules/check-entry', async(req,res) => {
  ...
  try {
    const {symbol, confidence, regime, entry_reason} = req.body;
    const reasons = [];
    let allowed = true;
    // Rule 1: Min confidence 80
    if (confidence < 8.0) { allowed=false; reasons.push(`Low confidence (${confidence}<8.0)`); }
    // Rule 2: Only BULL regime
    if (regime !== 'BULL') { allowed=false; reasons.push(`Not BULL regime (${regime})`); }
    // Rule 3: Check coin historical WR (only if >= 3 trades exist for this coin)
    const coinData = await db.execute({
      sql: `SELECT COUNT(*) as total,
                   COUNT(CASE WHEN outcome='WIN' THEN 1 END) as wins
            FROM trade_journal WHERE symbol=? AND outcome IS NOT NULL`,
      args: [symbol]
    });
    if (coinData.rows.length && coinData.rows[0].total >= 3) {
      const wr = (coinData.rows[0].wins / coinData.rows[0].total * 100);
      if (wr < 50) { allowed=false; reasons.push(`Low coin WR (${wr.toFixed(0)}%<50%)`); }
    }
...
// Trade Exit Alert — send to Telegram when trade closes
app.post('/api/trades/alert-exit', async(req,res)=>{
  if(!TG_TOKEN || !TG_CHAT) return res.json({ok:true});
  try {
    const {symbol, direction, entry_price, exit_price, pnl_pct, reason} = req.body;
    const dir = direction === 'LONG' ? '🟢 LONG' : '🔴 SHORT';
    const emoji = pnl_pct >= 0 ? '✅' : '🛑';
    const msg = `${emoji} <b>EXIT</b> ${dir} <b>${symbol}</b>
Entry: $${parseFloat(entry_price).toFixed(4)} → Exit: $${parseFloat(exit_price).toFixed(4)}
P&L: <b>${pnl_pct >= 0 ? '+' : ''}${pnl_pct.toFixed(2)}%</b> | Reason: ${reason}`;
    await sendTelegram(msg);
    res.json({ok:true});
  } catch(e) {
    console.warn('[TRADE ALERT EXIT]', e.message);
    res.json({ok:false, error:e.message});
  }
});
