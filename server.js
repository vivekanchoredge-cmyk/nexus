...
// ── OKX DATA FETCH ────────────────────────────────────────────────
// OKX interval map: 15m, 1H, 4H, 1D
const OKX_INTERVAL_MAP = {'15min':'15m','1hour':'1H','4hour':'4H','1day':'1D'};

async function loadOkxCandles(sym, interval, limit=100){
  const bar = OKX_INTERVAL_MAP[interval] || '15m';
  const r = await axios.get('https://www.okx.com/api/v5/market/candles', {
    params: { instId:`${sym}-USDT`, bar, limit },
    timeout: 12000, headers: { Accept:'application/json' }
  });
  if(r.data.code !== '0') throw new Error(`OKX ${sym} ${interval}: ${r.data.msg}`);
...
async function getOkxTopCoins(limit=30){
  const r = await axios.get('https://www.okx.com/api/v5/market/tickers',{
    params:{ instType:'SPOT' }, timeout:10000
  });
  if(r.data.code !== '0') throw new Error('OKX tickers failed');
  const exclude=['USDT','USDC','BUSD','DAI','TUSD','3L','3S','UP','DOWN','BEAR','BULL'];
  return r.data.data
    .filter(t=>{
      if(!t.instId.endsWith('-USDT')) return false;
      const sym=t.instId.replace('-USDT','');
      if(exclude.some(e=>sym.includes(e))) return false;
      return parseFloat(t.volCcy24h||0)*parseFloat(t.last||0) > 5000000; // $5M+ volume
    })
    .sort((a,b)=>{
      const va=parseFloat(a.volCcy24h||0)*parseFloat(a.last||0);
      const vb=parseFloat(b.volCcy24h||0)*parseFloat(b.last||0);
      return vb-va;
    })
    .slice(0,limit)
    .map(t=>({ sym:t.instId.replace('-USDT',''), price:parseFloat(t.last), volume:parseFloat(t.volCcy24h||0)*parseFloat(t.last||0), change24h:parseFloat((parseFloat(t.open24h||t.last)>0?((parseFloat(t.last)-parseFloat(t.open24h))/parseFloat(t.open24h)*100):0).toFixed(2)) }));
}
...
async function initialLoad(){
  console.log('Loading top coins from KuCoin + OKX...');

  // ── Step 1: Get top coins from KuCoin ──
  let kuSyms=[];
  try {
    ...
    console.log(`KuCoin top coins: ${kuSyms.slice(0,8).join(', ')} ...`);
  } catch(e){
    console.error('KuCoin top coins failed:',e.message);
    kuSyms=['BTC','ETH','BNB','SOL','XRP','DOGE','ADA','AVAX','LINK','DOT'];
  }

  // ── Step 2: Get top coins from OKX ──
  let okxSyms=[];
  try {
    const okxTop = await getOkxTopCoins(30);
    okxSyms = okxTop.map(t=>t.sym);
    // Store OKX tickers for price data
    okxTop.forEach(t=>{ if(!store[t.sym]) store[t.sym]={tick:{},m15:[],h1:[],h4:[],d1:[],source:'okx'}; store[t.sym].tick={price:t.price,change24h:t.change24h,volume24h:t.volume,lastUpdated:Date.now()}; });
    console.log(`OKX top coins: ${okxSyms.slice(0,8).join(', ')} ...`);
  } catch(e){
    console.error('OKX top coins failed:',e.message);
  }

  // ── Step 3: Merge — union of both, deduplicated ──
  const merged = [...new Set([...kuSyms,...okxSyms])];
  activeSymbols = merged;
  console.log(`Total unique coins: ${activeSymbols.length} (KuCoin:${kuSyms.length} OKX:${okxSyms.length})`);

  // ── Step 4: Load candles — KuCoin first, OKX fallback ──
  let loaded=0;
  for(const sym of activeSymbols){
    if(!store[sym]) store[sym]={tick:{},m15:[],h1:[],h4:[],d1:[]};
    const isKu = kuSyms.includes(sym);
    const isOkx = okxSyms.includes(sym);
    try {
      if(isKu){
        ...
        store[sym].source='kucoin';
      } else if(isOkx){
        store[sym].m15=await loadOkxCandles(sym,'15min',100); await sleep(150);
        store[sym].h1=await loadOkxCandles(sym,'1hour',100);  await sleep(150);
        store[sym].h4=await loadOkxCandles(sym,'4hour',60);   await sleep(150);
        store[sym].d1=await loadOkxCandles(sym,'1day',60);    await sleep(150);
        store[sym].source='okx';
      }
      loaded++;
    } catch(e){ console.error(`Candles ${sym}:`,e.message); }
  }
...
app.get('/api/test-exchanges', async(req,res)=>{
  ...
  // Test Binance
  try {
    const r = await axios.get('https://api.binance.com/api/v3/ticker/price?symbol=BTCUSDT',{timeout:8000});
    results.binance = { ok:true, btc: r.data.price };
  } catch(e) {
    results.binance = { ok:false, error: e.response?.status+' '+e.message };
  }
  // Test Bybit
  try {
    const r = await axios.get('https://api.bybit.com/v5/market/tickers?category=spot&symbol=BTCUSDT',{timeout:8000});
    results.bybit = { ok:true, btc: r.data.result?.list?.[0]?.lastPrice };
  } catch(e) {
    results.bybit = { ok:false, error: e.response?.status+' '+e.message };
  }
  // KuCoin already working
  results.kucoin = { ok:true, note:'already working' };
  res.json(results);
});
