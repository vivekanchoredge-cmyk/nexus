...
async function getSimilarTrades(symbol, direction, limit=5) {
  ...
  try {
    const trades = await db.execute({
      sql: `SELECT symbol, direction, confidence, pnl_pct, outcome 
            FROM active_trades 
            WHERE symbol=? AND direction=? AND status='CLOSED'
...
async function calculateSystemScore(symbol, direction, claudeConfidence) {
  if (!process.env.TURSO_DATABASE_URL) return claudeConfidence;
  
  try {
    // Get similar trades for this symbol + direction
    const similar = await db.execute({
      sql: `SELECT outcome FROM active_trades 
            WHERE symbol=? AND direction=? AND status='CLOSED'
    ...
    if (trades.length < 3) {
      // Not enough historical data - trust Claude
      return claudeConfidence;
    }
    
    // Calculate historical accuracy for this specific setup
    const wins = trades.filter(t => t.outcome === 'WIN').length;
    const historicalAccuracy = wins / trades.length; // 0.0 to 1.0
    
    // System score = Claude × Historical
    // If Claude says 8.0 but setup won 40% = 8.0 * 0.4 = 3.2
    // If Claude says 7.0 and setup won 80% = 7.0 * 0.8 = 5.6
    ...
    return Math.max(4.0, Math.min(10.0, systemScore)); // Bounds: 4.0 - 10.0
  } catch(e) {
    console.error('[SYSTEM SCORE] Error:', e.message);
    return claudeConfidence;
  }
}
...
async function generateFeedbackContext(symbol, direction, entry_price) {
  ...
  try {
    const similar = await db.execute({
      sql: `SELECT entry_price, pnl_pct, outcome, confidence 
            FROM active_trades 
            WHERE symbol=? AND direction=? AND status='CLOSED'
    ...
    if (wins.length > 0) {
      ...
      feedback += `✅ Wins: ${wins.length}/5 | Avg confidence: ${avgWinConf} | Avg PnL: ${avgWinPnL}%\n`;
    }
    ...
    if (losses.length > 0) {
      ...
      feedback += `❌ Losses: ${losses.length}/5 | Avg confidence: ${avgLossConf} | Avg Loss: ${avgLossPnL}%\n`;
    }
...
async function calculateSetupKelly(symbol, direction) {
  if (!process.env.TURSO_DATABASE_URL) return 1.0; // Default 1x
  
  try {
    const setupTrades = await db.execute({
      sql: `SELECT outcome, pnl_pct FROM active_trades 
            WHERE symbol=? AND direction=? AND status='CLOSED'
    ...
    if (trades.length < 3) return 1.0; // Need min 3 trades
    
    const wins = trades.filter(t => t.outcome === 'WIN').length;
    const winRate = wins / trades.length;
    
    // Per-setup Kelly: 0.3x (bad setup) to 1.5x (proven setup)
    let setupKelly = 0.3 + (winRate * 1.2);
    setupKelly = Math.max(0.3, Math.min(1.5, setupKelly));
    
    console.log(`[SETUP KELLY] ${symbol} ${direction}: ${(winRate*100).toFixed(0)}% WR → ${setupKelly.toFixed(2)}x size`);
