...
async function initDB() {
  ...
  // Clean up orphaned NULL PENDING signals older than 48h — these are noise
  await db.execute(`
    DELETE FROM signals
  ...
  `);
  // Mark very old PENDING signals (>72h) as EXPIRED so they don't pollute memory
  await db.execute(`
    UPDATE signals
    SET outcome = 'EXPIRED',
    ...
    const symRows = await db.execute({
      sql: `SELECT symbol,signal,entry,tp,sl,confidence,outcome,pnl_pct,reasoning,created_at
            ...
            WHERE outcome IN ('WIN','LOSS','EXPIRED')
              AND symbol=?
              AND (pnl_pct IS NOT NULL OR outcome='EXPIRED')
...
// ── DB CLEANUP (clean NULL/garbage signals) ───────────────────────
app.post('/api/db/cleanup', async(req,res) => {
  ...
  try {
    // Delete PENDING signals with NULL entry (never properly saved)
    const d1 = await db.execute({
      sql: `DELETE FROM signals WHERE outcome='PENDING' AND entry IS NULL`,
      args: []
    });
    // Mark stale PENDING (>48h) as EXPIRED
    const d2 = await db.execute({
      sql: `UPDATE signals SET outcome='EXPIRED', evaluated_at=CURRENT_TIMESTAMP
            WHERE outcome='PENDING' AND created_at < datetime('now','-48 hours')`,
      args: []
    });
    // Delete duplicate PENDING for same symbol+signal within 5 minutes
    const d3 = await db.execute({
      sql: `DELETE FROM signals WHERE id NOT IN (
              SELECT MIN(id) FROM signals
              WHERE outcome='PENDING'
              GROUP BY symbol, signal, CAST(strftime('%s',created_at)/300 AS INT)
            ) AND outcome='PENDING' AND entry IS NULL`,
      args: []
    });
    res.json({ok:true,
...
app.post('/api/trades/open', async(req,res) => {
  ...
  try {
    const {signal_id,symbol,direction,entry_price,tp_price,sl_price,sl_pct,tp_pct,
           confidence,size_usdt,reasoning,kucoin_entry_id,kucoin_sl_id,kucoin_tp_id} = req.body;
    ...
    const result = await db.execute({
      sql: `INSERT INTO active_trades
              (signal_id,symbol,direction,entry_price,tp_price,sl_price,sl_pct,tp_pct,
               confidence,size_usdt,reasoning,kucoin_entry_id,kucoin_sl_id,kucoin_tp_id,status)
            VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,'OPEN')`,
