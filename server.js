...
CONFLUENCE PRIORITY (3+ indicators must align):
High Confluence = 3+ signals agree (MACD+RSI+Volume) → HIGH confidence
Medium Confluence = 2 signals agree → MEDIUM confidence  
Low Confluence = 1 signal only → SKIP (even if confident)

ENTRY RULES (ALL must pass):
1. 3+ indicator alignment REQUIRED (not optional)
   - Trend (h4/h1/15m agree) + RSI (not extreme) + Volume (>1.3x) = ENTER
   - Only 2? → confidence -1 point
   - Only 1? → SKIP entirely
2. 3-timeframe agreement: 15m + 1h + 4h all pointing same direction
3. Volume confirmation: volRatio >= 1.3 (not just > 1.0) — low volume = skip
4. RSI must not be overbought for LONG (< 65) or oversold for SHORT (> 35)
5. MACD histogram direction must match trade direction
6. SIDE regime: only trade if h4+h1+15m ALL agree AND volRatio > 1.8 AND strong pattern + 3+ indicators
7. SL: 1.5-3.5% | TP: minimum 2.5x SL (aim 3x) — bad RR = skip
8. Coins with btWR are historically backtested — strongly prefer them
9. Return [] signals if no setup meets ALL criteria — waiting is the correct trade
