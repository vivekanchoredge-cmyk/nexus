...
async function loadMemory(symbol) {
  ...
  try {
    // Last 20 signals for this symbol + general last 10
    ...
    const lines = rows.rows.map(r => {
      ...
      return `[${r.created_at}] ${r.symbol} ${r.signal} entry=${r.entry} tp=${r.tp} sl=${r.sl} conf=${r.confidence}% => ${r.outcome} ${pnl} | reasoning: ${r.reasoning||'N/A'}`;
    }).join('\n');

    return `\n\nYOUR PAST SIGNAL HISTORY (learn from these mistakes and wins):\n${lines}\n\nBased on the above history: avoid repeating mistakes, do not re-enter losing patterns, increase confidence only when past similar setups won.`;
  } catch(e) {
...
app.listen(PORT, async () => {
  ...
  // Evaluate pending signals every 15 min
  setInterval(evaluatePendingSignals, 15 * 60 * 1000);

  // Full candle reload every 15min
