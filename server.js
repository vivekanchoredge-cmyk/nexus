  ...
  setInterval(async () => {
    try {
      ...
      candidates = candidates.filter(c=>{
        ...
        if(regime==='BULL'&&c.direction==='SHORT'){ console.log(`[AUTO] SKIP ${c.symbol} SHORT - BULL`); return false; }
        return true;
      });
      ...
      const prompt = `Regime:${regime}. GlobalWR:${recentStats.winRate}%(${recentStats.total}trades).\nAnalyze:\n${learningLines.join('\n')}\nPick best 1-2 ONLY if confidence>=7.0. Prefer coins with positive history. Skip weak setups. ${regime==='BEAR'?'SHORT only.':regime==='BULL'?'LONG only.':''}`;
      const r = await axios.post('https://api.anthropic.com/v1/messages',{
