...
async function initialLoad(){
  ...
  // Store BingX prices for common coins — spread comparison
  global.bxPrices = {};
  try {
    const bxTickers = await fetchBingXTickers();
    bxTickers.forEach(t => {
      const sym = t.symbol?.replace('-USDT','');
      if(sym) global.bxPrices[sym] = {
        bid: parseFloat(t.bidPrice||t.openPrice||0),
        ask: parseFloat(t.askPrice||t.openPrice||0),
        price: parseFloat(t.lastPrice||t.openPrice||0)
      };
    });
  } catch(e) { console.warn('BingX prices fetch failed:', e.message); }
...
app.post('/api/order',async(req,res)=>{
  ...
  try {
    ...
    // Smart exchange selection — better spread wala use karo
    let useBingX = bxOk && !kuOk; // default: KuCoin prefer
    if(kuOk && bxOk) {
      // Dono available hain — spread compare karo
      const bxData = global.bxPrices?.[symbol];
      const kuData = marketCache.find(c=>c.symbol===symbol);
      if(bxData && kuData && bxData.bid > 0 && kuData.bid > 0) {
        const kuSpread = (kuData.ask - kuData.bid) / kuData.bid * 100;
        const bxSpread = (bxData.ask - bxData.bid) / bxData.bid * 100;
        useBingX = bxSpread < kuSpread; // tighter spread = better
        console.log(`[EXCHANGE] ${symbol}: KU spread=${kuSpread.toFixed(4)}% BX spread=${bxSpread.toFixed(4)}% → using ${useBingX?'BingX':'KuCoin'}`);
      } else if(global.bxOnlyCoins?.has(symbol)) {
        useBingX = true; // BingX-only coin
      }
    }
