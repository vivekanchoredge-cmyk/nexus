...
// ── DYNAMIC KELLY: Calculate based on recent win rate ──────────────────
async function calculateDynamicKelly(symbol) {
  if (!process.env.TURSO_DATABASE_URL) return 2.0; // Default 2x
  try {
    // Last 20 trades
    const trades = await db.execute({
    ...
    if (!trades.rows || trades.rows.length < 5) return 2.0; // Need min 5 trades
    
    const wins = trades.rows.filter(t => t.outcome === 'WIN').length;
    const winRate = wins / trades.rows.length;
    
    // Kelly % = (BP - Q) / B
    // Simplified: if 40% WR → 0.5x, 50% → 1.0x, 60% → 1.5x, 70% → 2.0x
    let kellyMultiplier = 0.5 + (winRate - 0.40) * 3.33;
    kellyMultiplier = Math.max(0.3, Math.min(2.5, kellyMultiplier)); // Bounds: 0.3x - 2.5x
    
    console.log(`[KELLY] WinRate: ${(winRate*100).toFixed(1)}% → ${kellyMultiplier.toFixed(2)}x`);
    return kellyMultiplier;
  } catch(e) {
    console.error('[KELLY] Calculation failed:', e.message);
    return 2.0; // Default
  }
}
...
app.post('/api/analyze',async(req,res)=>{
  ...
  try {
    ...
    if(promptSymbol !== 'N/A') {
      ...
      dynamicKelly = await calculateDynamicKelly(promptSymbol);
    ...
    if(process.env.TURSO_DATABASE_URL&&Array.isArray(parsed.signals)&&parsed.signals.length>0){
      for(const sig of parsed.signals){
        ...
        try {
          ...
          // 🔥 REAL FIX: Feedback-based confidence adjustment (DIRECT injection)
          ...
          try {
            ...
            if (similarTrades.length >= 3) {
              ...
              if (winRate > 0.65) {
                finalConfidence += 1.5; // Strong history → boost
              } else if (winRate > 0.55) {
                finalConfidence += 0.5; // Positive history → slight boost
              } else if (winRate < 0.35) {
                finalConfidence -= 1.5; // Poor history → reduce
              } else if (winRate < 0.45) {
                finalConfidence -= 0.5; // Negative history → slight reduce
              }
              
              finalConfidence = Math.max(4.0, Math.min(10.0, finalConfidence)); // Bounds
              
              console.log(`[FEEDBACK] ${sig.symbol} ${sig.direction}: WR=${(winRate*100).toFixed(0)}% (${wins}/${similarTrades.length}) | Claude=${sig.confidence} → Adjusted=${finalConfidence.toFixed(1)}`);
            } else {
              console.log(`[FEEDBACK] ${sig.symbol} ${sig.direction}: Not enough history (${similarTrades.length} trades)`);
            }
          } catch(feedbackErr) {
            console.error(`[FEEDBACK ERROR] ${sig.symbol} ${sig.direction}:`, feedbackErr.message);
          }
          
          // PER-SETUP KELLY: Individual setup sizing based on win rate
          let setupKelly = 1.0;
          try {
            const setupTrades = await db.execute({
              sql: `SELECT outcome FROM signals 
          ...
          console.log(`Signal saved: ${sig.symbol} ${sig.direction} | Final=${finalConfidence.toFixed(1)} | Kelly=${setupKelly.toFixed(2)}x`);
