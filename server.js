  ...
  try {
    ...
    if(tradeForSignal.rows.length > 0) {
      ...
      if(t.signal_id) {
        ...
        // No signal_id — create new signal record for this trade (all required fields)
        await db.execute({
          sql: `INSERT INTO signals (symbol, signal, entry, tp, sl, outcome, pnl_pct, confidence, exit_price, trade_type, exchange, created_at, evaluated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
          args: [t.symbol, t.direction, t.entry_price||null, t.tp_price||null, t.sl_price||null, 
                 outcome, pnl_pct||null, t.confidence||0, close_price||null, t.trade_type||'spot', t.exchange||'kucoin']
        });
      }
    }
  ...
  try {
    ...
    if(tradeForSignal2.rows.length > 0) {
      ...
      if(t2.signal_id) {
        ...
        // No signal_id — create new signal record
        await db.execute({
          sql: `INSERT INTO signals (symbol, signal, entry, tp, sl, outcome, pnl_pct, confidence, exit_price, trade_type, exchange, created_at, evaluated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
          args: [symbol, t2.direction, t2.entry_price||null, t2.tp_price||null, t2.sl_price||null,
                 outcome2, pnl_pct||null, t2.confidence||0, close_price||null, t2.trade_type||'spot', t2.exchange||'kucoin']
        });
      }
    }
...
app.get('/api/signals/history', async(req,res) => {
  ...
  try {
    // signals table se (signal_id wale trades)
    ...
    // active_trades se bhi lo (signal_id NULL wale — paper trading)
    // BUT: already-inserted signals ko exclude karo (no duplicate counting)
    ...
    // Merge dono — no duplicates (signal_id NOT IN signals.id ensures that)
