...
app.post('/api/prefilter',(req,res)=>{
  ...
  // 🔥 REGIME FILTER: Block BEAR+LONG and BULL+SHORT at prefilter level
  // Get BTC market regime
  const btc = marketCache.find(c=>c.symbol==='BTC');
  const btcTrend = btc?.d1?.trend || 'UP';
  const btcST = btc?.d1?.supertrend?.trend || 'UP';
  const marketRegime = (btcTrend === 'DOWN' && btcST === 'DOWN') ? 'BEAR' 
                     : (btcTrend === 'UP' && btcST === 'UP') ? 'BULL' 
                     : 'SIDE';
...
app.listen(PORT,async()=>{
  ...
  setInterval(async () => {
    try {
      ...
      const btcTrend = btc?.d1?.trend||'UP';
      const btcST = btc?.d1?.supertrend?.trend||'UP';
      const regime = (btcTrend==='DOWN'&&btcST==='DOWN')?'BEAR':(btcTrend==='UP'&&btcST==='UP')?'BULL':'SIDE';
