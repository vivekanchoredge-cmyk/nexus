...
${symLines || '  No signals yet for this coin'}
...
- If a coin has <30% win rate in history, lower your confidence or SKIP
- If you previously lost on this exact setup (same pattern+trend), do NOT repeat
- If avg PnL is negative for a coin, avoid it unless setup is exceptional
- Increase confidence only on setups that historically won`
  } catch(e) {
    ...
    for (const row of rows.rows) {
      ...
      if (outcome) {
        ...
        await db.execute({
          ...
          args: [outcome, exitPrice, pnlPct, row.id]
        });
...
// ── TRADING MODE ──────────────────────────────────────────────────
let tradingMode = process.env.TRADING_MODE || 'paper'; // 'paper' or 'live'
...
async function placeMarketOrder(symbol, side, sizeUsdt) {
  ...
  const body = JSON.stringify({
    ...
    symbol:   `${symbol}-USDT`,
    type:     'market',
    funds:    sizeUsdt.toFixed(2)  // spend this much USDT
  });
...
app.post('/api/analyze', async (req,res) => {
  ...
  try {
    ...
    const symMatch = req.body.prompt?.match(/\b([A-Z]{2,10})\b/);
    const promptSymbol = symMatch?.[1] || 'N/A';
...
${memory}`,
    ...
    // Save signal to Turso as PENDING
    if (process.env.TURSO_DATABASE_URL) {
      try {
        await db.execute({
...
function requireAuth(req, res, next) {
  ...
  // Also allow if no password is set (fallback)
  if (!process.env.NEXUS_PASSWORD) return next();
...
app.post('/api/order/close', async (req,res) => {
  ...
  try {
    ...
    const closeId = await placeMarketOrder(symbol, closeSide, qty * (marketCache.find(c=>c.symbol===symbol)?.price||1));
