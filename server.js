...
1. 3-timeframe alignment required: 15m + 1h + 4h all pointing same direction
2. Volume confirmation: volRatio >= 1.3 (not just > 1.0) — low volume entries fail
3. RSI must not be overbought for LONG (< 65) or oversold for SHORT (> 35)
4. MACD histogram direction must match trade direction
5. SIDE regime: only trade if h4+h1+15m ALL agree AND volRatio > 1.8 AND strong pattern
6. SL: 1.5-3.5% | TP: minimum 2.5x SL (aim 3x) — bad RR = skip
7. Coins with btWR are historically backtested — prefer them strongly
8. Return [] if no setup meets ALL criteria — waiting is the correct trade
