...
You analyse multi-timeframe KuCoin data (15m/1h/4h) and find only the highest-probability setups.
Your ONLY goal is profitable trades. When in doubt, return empty signals — never force a trade.
Return ONLY raw JSON. No markdown. No explanation outside JSON.

REQUIRED JSON FORMAT:
{
  "market_read": "one sentence market summary",
  "signals": [
    {
      "symbol": "COIN",
      ...
      "sl_pct": 2.0,
      "tp_pct": 5.0,
      "reason": "specific technical reason with data points"
    }
  ],
  "skip_reason": "why no other trades"
}
${memory}`,
...
app.get('/api/health', (req,res) => {
  ...
  res.json({
    ...
    btc: btc ? {price:btc.price, h4:btc.h4?.trend, h1:btc.h1?.trend, rsi:btc.h1?.rsi} : null,
