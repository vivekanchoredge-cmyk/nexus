  ...
  try {
    ...
    if(tradeForSignal.rows.length > 0) {
      ...
      if(t.signal_id) {
        ...
        // No signal_id — create new signal record for this trade
        await db.execute({
          sql: `INSERT INTO signals (symbol, signal, outcome, pnl_pct, confidence, exit_price, created_at, evaluated_at)
                VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
          args: [t.symbol, t.direction, outcome, pnl_pct||null, t.confidence||0, close_price||null]
        });
      }
    }
    ...
    if(tradeForSignal2.rows.length > 0) {
      ...
      if(t2.signal_id) {
        ...
        await db.execute({
          sql: `INSERT INTO signals (symbol, signal, outcome, pnl_pct, confidence, exit_price, created_at, evaluated_at)
                VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
          args: [symbol, t2.direction, outcome2, pnl_pct||null, t2.confidence||0, close_price||null]
        });
      }
    }
