...
${memory}`,
    ...
    // Save signal to Turso only if it's a real LONG or SHORT signal
    const isRealSignal = ['LONG','SHORT'].includes(parsed.signal);
    if (process.env.TURSO_DATABASE_URL && isRealSignal && parsed.symbol && parsed.symbol !== 'N/A') {
      try {
        await db.execute({
