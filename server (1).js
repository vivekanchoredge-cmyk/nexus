...
function macd(closes){
  ...
  const macdArr=closes.slice(-35).map((_,i,a)=>{
...
function stochRsi(closes,n=14,smooth=3){
  if(!closes||closes.length<n*2) return {k:50,d:50};
  const rsiArr=[];
  ...
  const rawK=maxR===minR?50:((rsiArr.at(-1)-minR)/(maxR-minR))*100;
  const kArr=rsiArr.slice(-smooth).map((_,i,a)=>{
    const sl=a.slice(Math.max(0,i-smooth+1),i+1);
    const mn=Math.min(...sl),mx=Math.max(...sl);
    return mx===mn?50:((a[i]-mn)/(mx-mn))*100;
  });
  const k=parseFloat(rawK.toFixed(1));
  const d=parseFloat((kArr.reduce((a,b)=>a+b,0)/kArr.length).toFixed(1));
  return {k,d};
}
...
function pattern(candles){
  ...
  const body=Math.abs(c3.close-c3.open), range=c3.high-c3.low, bull=c3.close>c3.open;
...
function tfSnapshot(candles){
  ...
  return {
    ...
    stochD:   sr.d,
    macdHist: mc.hist,
    macdBull: mc.hist>0&&mc.macd>mc.signal,
    bb:       bb,
...
// Returns score 0-100. Only call Claude if score >= threshold
function preFilter(coin, aggressive=false) {
  ...
  if(longSetup){
    ...
    // MACD confirmation
    if(h1.macdBull){ score+=15; reasons.push('MACD bull'); }
    // Bollinger position (not too high)
    if(h1.bbPos<75){ score+=10; reasons.push(`BB pos ${h1.bbPos}%`); }
    // Stochastic not overbought
    if(h1.stochK<75){ score+=10; reasons.push(`Stoch ${h1.stochK}`); }
    // Volume
    ...
    // Pattern bonus
    if(['BULL_ENGULF','HAMMER','3_GREEN'].includes(m15.pattern)){ score+=15; reasons.push(`Pattern ${m15.pattern}`); }
  } else if(shortSetup){
    ...
    if(!h1.macdBull){ score+=15; reasons.push('MACD bear'); }
    if(h1.bbPos>25){ score+=10; reasons.push(`BB pos ${h1.bbPos}%`); }
    if(h1.stochK>25){ score+=10; reasons.push(`Stoch ${h1.stochK}`); }
    if(m15.volRatio>1.5){ score+=15; reasons.push(`Vol ${m15.volRatio}x strong`); }
    else if(m15.volRatio>1.2){ score+=8; reasons.push(`Vol ${m15.volRatio}x ok`); }
    if(['BEAR_ENGULF','3_RED'].includes(m15.pattern)){ score+=15; reasons.push(`Pattern ${m15.pattern}`); }
  } else {
